...
// v4.1: Earlier all the dimensions (length/width/thickness) of transistors and
// wires and all delays were calculated for the 0.8 micron process and then
// scaled to the input techology. Now all dimensions and delays are calculated
// directly for the input technology.
...
void compute_device_widths(int C, int B, int A, int fullyassoc, int Ndwl,
                           int Ndbl, double Nspd) {
  int rows, cols, numstack, l_predec_nor_v, l_predec_nor_h;
  double desiredrisetime, Rpdrive, Cline, Cload;
  double effWdecNORn, effWdecNORp, effWdec3to8n, effWdec3to8p, wire_res,
      wire_cap;
  ...
  double previous_ndriveW = 0, previous_pdriveW = 0, current_ndriveW = 0,
         current_pdriveW = 0;
  int i;

  // v4.1: Fixing double->int type conversion problems. EPSILON is added below
  // to make sure the final int value is the correct one rows =
  // C/(CHUNKSIZE*B*A*Ndbl*Nspd);
  ...
  /*dt: I'm changing this back to what's in CACTI (as opposed to eCacti), i.e.
   * counting the short poly connection to the pass transistors*/
  Cline =
      (gatecappass(Wmemcella, (BitWidth - 2 * Wmemcella) / 2.0) +
       gatecappass(Wmemcella, (BitWidth - 2 * Wmemcella) / 2.0) + Cwordmetal) *
      cols;
  ...
  numstack =
      (int)ceil((1.0 / 3.0) *
                logtwo((double)((double)C / (double)(B * A * Ndbl * Nspd))));
  if (numstack == 0)
    numstack = 1;
  ...
  /*dt: The *8 is there because above we mysteriously divide the capacity in
   * BYTES by the number of BITS per wordline */
  l_predec_nor_v = rows * CHUNKSIZE;
  /*dt: If we follow the original drawings from the TR's, then there is almost
  no horizontal wires, only the poly for contacting the nor gates. The poly part
  we don't model right now */
  l_predec_nor_h = 0;
  ...
  Cline = gatecap(WdecNORn + WdecNORp,
                  ((numstack * 40 / FUDGEFACTOR) + 20.0 / FUDGEFACTOR)) *
              rows / 8 +
          GlobalCbitmetal * (l_predec_nor_v) +
          GlobalCwordmetal * (l_predec_nor_h);
  ...
  if (Ndwl * Ndbl == 1) {
    ...
    Cline =
        4 * gatecap(previous_ndriveW + previous_pdriveW, 10.0 / FUDGEFACTOR) +
        GlobalCwordmetal * horizontal_edge;
    Cload = Cline / gatecap(1.0, 0.0);
    ...
    while (nr_subarrays_left > 1) {
      ...
      if (v_or_h) {
        ...
        Cdectreesegments[nr_dectreesegments - 1] =
            GlobalCbitmetal * vertical_step;
        Rdectreesegments[nr_dectreesegments - 1] =
            0.5 * GlobalRbitmetal * vertical_step;
        Cline = gatecap(previous_ndriveW + previous_pdriveW, 0) +
                GlobalCbitmetal * vertical_step;
        v_or_h = 0;
        vertical_step *= 2;
        nr_subarrays_left /= 2;
        ...
        Cdectreesegments[nr_dectreesegments - 1] =
            GlobalCwordmetal * horizontal_step;
        Rdectreesegments[nr_dectreesegments - 1] =
            0.5 * GlobalRwordmetal * horizontal_step;
        Cline = gatecap(previous_ndriveW + previous_pdriveW, 0) +
                GlobalCwordmetal * horizontal_step;
        v_or_h = 1;
        horizontal_step *= 2;
    ...
    if (nr_dectreesegments >= 10) {
      printf("Too many segments in the data decoder H-tree. Overflowing the "
             "preallocated array!");
      exit(1);
    }
    wire_cap =
        GlobalCbitmetal * v_inv_predecode + GlobalCwordmetal * h_inv_predecode;
    wire_res = 0.5 * (GlobalRbitmetal * v_inv_predecode +
                      GlobalRwordmetal * h_inv_predecode);
  }
  ...
  Wdecdrivep_first =
      2 * (Wdecdriven_second + Wdecdrivep_second) * SizingRatio / 3;
  ...
  if (!fullyassoc) {
    ...
    if (!force_tag) {
      ...
      tagbits =
          ADDRESS_BITS + EXTRA_TAG_BITS - (int)(logtwo((double)B) + EPSILON);
    } else {
  ...
  if (Ndwl * Ndbl == 1) {
    ...
    Cline = gatecap(Wsenseextdrv1n + Wsenseextdrv1p, 10.0 / FUDGEFACTOR) +
            GlobalCwordmetal * cols;
    Cload = Cline / gatecap(1.0, 0.0);
    ...
    while (nr_subarrays_left > 1) {
      ...
      if (v_or_h) {
        ...
        Coutdrvtreesegments[nr_outdrvtreesegments - 1] =
            GlobalCbitmetal * vertical_step;
        Routdrvtreesegments[nr_outdrvtreesegments - 1] =
            0.5 * GlobalRbitmetal * vertical_step;
  ...
  if (nr_outdrvtreesegments >= 20) {
    printf("Too many segments in the output H-tree. Overflowing the "
           "preallocated array!");
    exit(1);
  }
  // typical width of gate considered for the estimating val of draincap.
  Cload = gatecap(previous_ndriveW + previous_pdriveW, 0) +
          Coutdrvtreesegments[0] +
          (draincap(5.0 / FUDGEFACTOR, NCH, 1) +
           draincap(5.0 / FUDGEFACTOR, PCH, 1)) *
              A * muxover;
...
void compute_tag_device_widths(int C, int B, int A, int Ntspd, int Ntwl,
                               int Ntbl, double NSubbanks) {

  int rows, cols, tagbits, numstack, l_predec_nor_v, l_predec_nor_h;
  double Cline, Cload, Rpdrive, desiredrisetime;
  double effWtdecNORn, effWtdecNORp, effWtdec3to8n, effWtdec3to8p, wire_res,
      wire_cap;
  int horizontal_edge = 0;
  int nr_subarrays_left = 0, v_or_h = 0;
  int horizontal_step = 0, vertical_step = 0;
  ...
  if (!force_tag) {
    // v4.1: Fixing double->int type conversion problems. EPSILON is added below
    // to make sure the final int value is the correct one tagbits =
    // ADDRESS_BITS +
    // EXTRA_TAG_BITS-(int)logtwo((double)C)+(int)logtwo((double)A)-(int)(logtwo(NSubbanks));
    tagbits =
        (int)(ADDRESS_BITS + EXTRA_TAG_BITS - (int)logtwo((double)C) +
  ...
  /*dt: changing the calculations for the tag wordline to be the same as for the
   * data wordline*/
  /* Use a first-order approx */
  desiredrisetime = krise * log((double)(cols)) / 2.0;
  ...
  numstack =
      (int)ceil((1.0 / 3.0) *
                logtwo((double)((double)C / (double)(B * A * Ntbl * Ntspd))));
  if (numstack == 0)
    numstack = 1;
  if (numstack > 5)
    numstack = 5;
  ...
  Cline = gatecap(WtdecNORn + WtdecNORp,
                  ((numstack * 40) / FUDGEFACTOR + 20.0 / FUDGEFACTOR)) *
              rows / 8 +
          GlobalCbitmetal * (l_predec_nor_v) +
          GlobalCwordmetal * (l_predec_nor_h);
  ...
  if (Ntwl * Ntbl == 1) {
    ...
    while (nr_subarrays_left > 1) {
      ...
      if (v_or_h) {
        v_inv_predecode += vertical_step;
        Ctdectreesegments[nr_tdectreesegments - 1] =
            GlobalCbitmetal * vertical_step;
        Rtdectreesegments[nr_tdectreesegments - 1] =
            0.5 * GlobalRbitmetal * vertical_step;
        Cline = gatecap(previous_ndriveW + previous_pdriveW, 0) +
                GlobalCbitmetal * vertical_step;
        v_or_h = 0;
    ...
    if (nr_tdectreesegments >= 10) {
      printf("Too many segments in the tag decoder H-tree. Overflowing the "
             "preallocated array!");
      exit(1);
    }
  ...
  Wtdecdriven_first =
      (Wtdecdriven_second + Wtdecdrivep_second) * SizingRatio / 3;
  Wtdecdrivep_first =
      2 * (Wtdecdriven_second + Wtdecdrivep_second) * SizingRatio / 3;
}
double cmos_ileakage(double nWidth, double pWidth, double nVthresh_dual,
                     double nVthreshold, double pVthresh_dual,
                     double pVthreshold) {
  double leakage = 0.0;
  ...
  if (have_leakage_params) {
    ...
    if (dualVt == TRUE) {
      ...
      if ((cached_nmos_thresh == nVthresh_dual) &&
          (cached_pmos_thresh == pVthresh_dual) && valid_cache) {
        leakage = nWidth * norm_nmos_leakage + pWidth * norm_pmos_leakage;
      } else {
        leakage = simplified_cmos_leakage(
            nWidth * inv_Leff, pWidth * inv_Leff, nVthresh_dual, pVthresh_dual,
            &norm_nmos_leakage, &norm_pmos_leakage);
      ...
      if ((cached_nmos_thresh == nVthreshold) &&
          (cached_pmos_thresh == pVthreshold) && valid_cache) {
        leakage = nWidth * norm_nmos_leakage + pWidth * norm_pmos_leakage;
      } else {
        leakage = simplified_cmos_leakage(
            nWidth * inv_Leff, pWidth * inv_Leff, nVthreshold, pVthreshold,
            &norm_nmos_leakage, &norm_pmos_leakage);
...
double objective_function(double delay_weight, double area_weight,
                          double power_weight, double delay, double area,
                          double power) {
  return (double)(area_weight * area + delay_weight * delay +
                  power_weight * power);
}
...
void subbank_routing_length(int C, int B, int A, char fullyassoc, int Ndbl,
                            double Nspd, int Ndwl, int Ntbl, int Ntwl,
                            int Ntspd, double NSubbanks, double *subbank_v,
                            double *subbank_h) {
  double htree;
  ...
  if (!fullyassoc) {
    ...
    if (Ndwl * Ndbl == 1) {
      ...
      // v4.1: Fixing double->int type conversion problems. EPSILON is added
      // below to make sure
      // the final int value is the correct one
      // htree_int = (int) htree;
    ...
    if (!force_tag) {
      // v4.1: Fixing double->int type conversion problems. EPSILON is added
      // below to make sure the final int value is the correct one tagbits =
      // ADDRESS_BITS + EXTRA_TAG_BITS - (int) logtwo ((double) C) + (int)
      //logtwo ((double) A) - (int) (logtwo (NSubbanks));
      tagbits =
          (int)(ADDRESS_BITS + EXTRA_TAG_BITS - (int)logtwo((double)C) +
                (int)logtwo((double)A) - (int)(logtwo(NSubbanks)) + EPSILON);
    } else {
...
void subbank_dim(int C, int B, int A, char fullyassoc, int Ndbl, int Ndwl,
                 double Nspd, int Ntbl, int Ntwl, int Ntspd, double NSubbanks,
  ...
  if (!fullyassoc) {
    ...
    // v4.1: Fixing double->int type conversion problems. EPSILON is added below
    // to make sure the final int value is the correct one cols_data_subarray =
    // (8 * B * A * Nspd / Ndwl); rows_data_subarray = (C / (B * A * Ndbl *
    // Nspd));
    cols_data_subarray = (int)(8 * B * A * Nspd / Ndwl + EPSILON);
    rows_data_subarray = (int)(C / (B * A * Ndbl * Nspd) + EPSILON);
    ...
    if (!force_tag) {
      // v4.1: Fixing double->int type conversion problems. EPSILON is added
      // below to make sure the final int value is the correct one
    ...
    *subbank_h = inter_h;
    ...
    if (!force_tag) {
      // v4.1: Fixing double->int type conversion problems. EPSILON is added
      // below to make sure the final int value is the correct one tagbits =
      // ADDRESS_BITS + EXTRA_TAG_BITS - (int) logtwo ((double) B);
      tagbits =
          ADDRESS_BITS + EXTRA_TAG_BITS - (int)(logtwo((double)B) + EPSILON);
    } else {
      tagbits = force_tag_size;
...
void subbanks_routing_power(char fullyassoc, int A, double NSubbanks,
  ...
  if (NSubbanks == 4.0) {
    ...
    Ceq =
        draincap(Wdecdrivep, PCH, 1) + draincap(Wdecdriven, NCH, 1) + wire_cap;
    dynPower += 2 * ADDRESS_BITS * Ceq * .5 * VddPow * VddPow;

    /* calculation of out driver power */
    wire_cap_outdrv = Cbitmetal * (*subbank_v);
    Ceq_outdrv = draincap(Wsenseextdrv1p, PCH, 1) +
                 draincap(Wsenseextdrv1n, NCH, 1) +
                 gatecap(Wsenseextdrv2n + Wsenseextdrv2p, 10.0 / FUDGEFACTOR);
    dynPower += 2 * Ceq_outdrv * VddPow * VddPow * .5 * BITOUT * A * muxover;
    Ceq_outdrv = draincap(Wsenseextdrv2p, PCH, 1) +
                 draincap(Wsenseextdrv2n, NCH, 1) + wire_cap_outdrv;
    dynPower += 2 * Ceq_outdrv * VddPow * VddPow * .5 * BITOUT * A * muxover;
  }
...
double address_routing_delay(int C, int B, int A, char fullyassoc, int Ndwl,
                             int Ndbl, double Nspd, int Ntwl, int Ntbl,
                             int Ntspd, double *NSubbanks, double *outrisetime,
                             powerDef *power) {
  ...
  nextinputtime =
      horowitz(0.0, tf, VTHINV360x240, VTHINV360x240, FALL) / (VTHINV360x240);

  Ceq = draincap(Wdecdrivep, PCH, 1) + draincap(Wdecdriven, NCH, 1) +
        gatecap(Wdecdrivep + Wdecdriven, 0.0);
  tf = Ceq * transreson(Wdecdriven, NCH, 1);
  nextinputtime =
      horowitz(nextinputtime, tf, VTHINV360x240, VTHINV360x240, RISE) /
      (1.0 - VTHINV360x240);
  addr_h = 0;
  addr_v = 0;
  ...
  Cline = *NSubbanks * (gatecap(Wdecdrivep_first + Wdecdriven_first, 0.0) +
  ...
  delay_stage1 =
      horowitz(nextinputtime, tf, VTHINV360x240, VTHINV360x240, FALL);
  nextinputtime =
      horowitz(nextinputtime, tf, VTHINV360x240, VTHINV360x240, FALL) /
      (VTHINV360x240);
  ...
  lkgCurrent += ADDRESS_BITS * 0.5 *
                cmos_ileakage(Waddrdrvn2, Waddrdrvp2, Vt_bit_nmos_low, Vthn,
                              Vt_bit_pmos_low, Vthp);

  Ceq = draincap(Waddrdrvp1, PCH, 1) + draincap(Waddrdrvn1, NCH, 1) + wire_cap +
        *NSubbanks * (gatecap(Wdecdrivep_first + Wdecdriven_first, 0.0) +
                      gatecap(Wtdecdrivep_first + Wtdecdriven_first, 0.0));
  ...
  lkgCurrent += ADDRESS_BITS * 0.5 *
                cmos_ileakage(Waddrdrvn1, Waddrdrvp1, Vthn, Vt_bit_nmos_low,
                              Vt_bit_pmos_low, Vthp);
...
/*dt: this is integrated from eCACTI. But we use want Energy per operation, not
 * some measure of power, so no FREQ. */
double decoder_delay(int C, int B, int A, int Ndwl, int Ndbl, double Nspd,
                     double NSubbanks, double *Tdecdrive, double *Tdecoder1,
                     double *Tdecoder2, double inrisetime, double *outrisetime,
                     int *nor_inputs, powerDef *power) {
  int numstack, Nact;
  ...
  int addr_bits =
      (int)(logtwo((double)((double)C / (double)(B * A * Ndbl * Nspd))) +
            EPSILON);
  ...
  if (NSubbanks > 2) {
    ...
    Ceq = draincap(Wdecdrivep_first, PCH, 1) +
          draincap(Wdecdriven_first, NCH, 1) +
          gatecap(Wdecdrivep_first + Wdecdriven_first, 0.0);
  ...
  lkgCurrent += addr_bits_routed * 0.5 *
                cmos_ileakage(Wdecdriven_first, Wdecdrivep_first,
  ...
  *Tdecdrive += this_delay;
  ...
  if (nr_dectreesegments) {
    Ceq = draincap(Wdecdrivep_second, PCH, 1) +
          draincap(Wdecdriven_second, NCH, 1) +
          gatecap(3 * WdecdrivetreeN[nr_dectreesegments - 1], 0) +
          Cdectreesegments[nr_dectreesegments - 1];
    Req = transreson(Wdecdriven_second, NCH, 1) +
          Rdectreesegments[nr_dectreesegments - 1];
    ...
    *Tdecdrive += this_delay;
    ...
    lkgCurrent += addr_bits_routed * 0.5 *
                  cmos_ileakage(Wdecdriven_second, Wdecdrivep_second,
                                Vt_bit_nmos_low, Vthn, Vt_bit_pmos_low, Vthp) *
                  1.0 / (Ndwl * Ndbl);
  }
  ...
  for (i = nr_dectreesegments; i > 2; i--) {
    ...
    Ceq =
        (Cdectreesegments[i - 2] + draincap(2 * WdecdrivetreeN[i - 1], PCH, 1) +
         draincap(WdecdrivetreeN[i - 1], NCH, 1) +
         gatecap(3 * WdecdrivetreeN[i - 2], 0.0));
    Req = (Rdectreesegments[i - 2] + transreson(WdecdrivetreeN[i - 1], NCH, 1));
    tf = Req * Ceq;
    /*dt: This shouldn't be all falling, but interleaved. Have to fix that at
     * some point.*/
    this_delay = horowitz(inrisetime, tf, VTHINV360x240, VTHINV360x240, RISE);
    *Tdecdrive += this_delay;
    inrisetime = this_delay / (1.0 - VTHINV360x240);
    ...
    dynPower += addr_bits_routed * Ceq * .5 * VddPow * VddPow;
    lkgCurrent +=
        1.0 / (Ndwl * Ndbl) * pow(2, nr_dectreesegments - i) *
        addr_bits_routed * 0.5 *
        cmos_ileakage(WdecdrivetreeN[i - 1], 2 * WdecdrivetreeN[i - 1],
                      Vt_bit_nmos_low, Vthn, Vt_bit_pmos_low, Vthp);
  }

  if (nr_dectreesegments) {
    Ceq = 4 * gatecap(Wdec3to8n + Wdec3to8p, 10.0 / FUDGEFACTOR) +
          Cdectreesegments[0] + draincap(2 * WdecdrivetreeN[0], PCH, 1) +
          draincap(WdecdrivetreeN[0], NCH, 1);
    Rwire = Rdectreesegments[0];
    tf = (Rwire + transreson(2 * WdecdrivetreeN[0], PCH, 1)) * Ceq;

    dynPower += addr_bits_routed * Ceq * .5 * VddPow * VddPow;
    lkgCurrent += 1.0 / (Ndwl * Ndbl) * pow(2, nr_dectreesegments) *
                  addr_bits_routed * 0.5 *
                  cmos_ileakage(WdecdrivetreeN[0], 2 * WdecdrivetreeN[0],
                                Vt_bit_nmos_low, Vthn, Vt_bit_pmos_low, Vthp);
  } else {
    Ceq = 4 * gatecap(Wdec3to8n + Wdec3to8p, 10.0 / FUDGEFACTOR) +
          Cdectreesegments[0] + draincap(Wdecdrivep_second, PCH, 1) +
          draincap(Wdecdriven_second, NCH, 1);
    Rwire = Rdectreesegments[0];
    tf = (Rwire + transreson(Wdecdrivep_second, PCH, 1)) * Ceq;

    dynPower += addr_bits_routed * Ceq * .5 * VddPow * VddPow;
    lkgCurrent += 1.0 / (Ndwl * Ndbl) * addr_bits_routed * 0.5 *
                  cmos_ileakage(Wdecdriven_second, Wdecdrivep_second,
                                Vt_bit_nmos_low, Vthn, Vt_bit_pmos_low, Vthp);
  }
  ...
  *Tdecdrive += this_delay;
  ...
  lkgCurrent +=
      8 * 0.2 * 0.5 *
      cmos_ileakage(Wdec3to8n, Wdec3to8p, Vt_bit_nmos_low, Vthn,
                    Vt_bit_pmos_low, Vthp) *
  ...
  Ceq = 3 * draincap(Wdec3to8p, PCH, 1) + draincap(Wdec3to8n, NCH, 3) +
        gatecap(WdecNORn + WdecNORp,
                ((numstack * 40 / FUDGEFACTOR) + 20.0 / FUDGEFACTOR)) *
            rows / 8 +
        GlobalCbitmetal * (l_predec_nor_v) +
        GlobalCwordmetal * (l_predec_nor_h);
  Rwire = GlobalRbitmetal * (l_predec_nor_v) / 2 +
          GlobalRwordmetal * (l_predec_nor_h) / 2;
  ...
  lkgCurrent += 0.5 * 0.2 * rows *
                cmos_ileakage(WdecNORn, WdecNORp, Vt_bit_nmos_low, Vthn,
                              Vt_bit_pmos_low, Vthp);
  ...
  // dynPower+=Ndwl*Ndbl*Ceq*VddPow*VddPow*4*ceil((1.0/3.0)*logtwo(
  // (double)((double)C/(double)(B*A*Ndbl*Nspd))));
  dynPower += 0.5 * Nact * Ceq * VddPow * VddPow * 4 *
              ceil((1.0 / 3.0) *
                   logtwo((double)((double)C / (double)(B * A * Ndbl * Nspd))));
  ...
  *Tdecoder1 = horowitz(nextinputtime, tf, VTHNAND60x120, vth, RISE);
  ...
  lkgCurrent += 0.5 * rows *
                cmos_ileakage(Wdecinvn, Wdecinvp, Vt_bit_nmos_low, Vthn,
                              Vt_bit_pmos_low, Vthp);
  tf = Req * Ceq;
  ...
  *nor_inputs = numstack;
  ...
  // printf("%g %g %g %d %d %d\n",*Tdecdrive,*Tdecoder1,*Tdecoder2,Ndwl,
  // Ndbl,Nspd);

  // fprintf(stderr, "%f %f %f %f %d %d %d\n",
  // (*Tdecdrive+*Tdecoder1+*Tdecoder2)*1e3, *Tdecdrive*1e3, *Tdecoder1*1e3,
  // *Tdecoder2*1e3, Ndwl, Ndbl, Nspd);
  power->readOp.dynamic = dynPower;
...
/*dt: incorporating leakage code from eCacti, see decoder_delay for more
 * comments */
double decoder_tag_delay(int C, int B, int A, int Ntwl, int Ntbl, int Ntspd,
                         double NSubbanks, double *Tdecdrive, double *Tdecoder1,
                         double *Tdecoder2, double inrisetime,
                         double *outrisetime, int *nor_inputs,
                         powerDef *power) {
  // double Ceq,Req,Rwire,tf,nextinputtime,vth,tstep;
  double Ceq, Req, Rwire, tf, nextinputtime, vth;
  ...
  int addr_bits =
      (int)(logtwo((double)((double)C / (double)(B * A * Ntbl * Ntspd))) +
            EPSILON);
  int horizontal_edge = 0;
  int nr_subarrays_left = 0, v_or_h = 0;
  int horizontal_step = 0, vertical_step = 0;
  ...
  // v4.1: Fixing double->int type conversion problems. EPSILON is added below
  // to make sure the final int value is the correct one int routing_bits =
  // (int)logtwo( (double)((double)C/(double)B));
  int routing_bits = (int)(logtwo((double)((double)C / (double)B)) + EPSILON);
  int tag_bits_routed;

  rows = C / (8 * B * A * Ntbl * Ntspd);
  if (!force_tag) {
    // tagbits = ADDRESS_BITS +
    // EXTRA_TAG_BITS-(int)logtwo((double)C)+(int)logtwo((double)A)-(int)(logtwo(NSubbanks));
    tagbits =
        (int)(ADDRESS_BITS + EXTRA_TAG_BITS - (int)logtwo((double)C) +
              (int)logtwo((double)A) - (int)(logtwo(NSubbanks)) + EPSILON);
  ...
  lkgCurrent = 0.5 *
               cmos_ileakage(Wtdecdriven_first, Wtdecdrivep_first,
                             Vt_bit_nmos_low, Vthn, Vt_bit_pmos_low, Vthp) *
               1.0 / (Ntwl * Ntbl);
  ...
  *Tdecdrive = 0;
  ...
  /*dt: the first inverter driving a bigger inverter*/
  Ceq = draincap(Wtdecdrivep_first, PCH, 1) +
        draincap(Wtdecdriven_first, NCH, 1) +
  ...
  *Tdecdrive += this_delay;
  ...
  if (nr_tdectreesegments) {
    Ceq = draincap(Wtdecdrivep_second, PCH, 1) +
          draincap(Wtdecdriven_second, NCH, 1) +
          gatecap(3 * WtdecdrivetreeN[nr_tdectreesegments - 1], 0) +
          Ctdectreesegments[nr_tdectreesegments - 1];
    Req = transreson(Wtdecdriven_second, NCH, 1) +
          Rtdectreesegments[nr_tdectreesegments - 1];
    ...
    *Tdecdrive += this_delay;
    ...
    lkgCurrent += tag_bits_routed * 0.5 *
                  cmos_ileakage(Wtdecdriven_second, Wtdecdrivep_second,
                                Vt_bit_nmos_low, Vthn, Vt_bit_pmos_low, Vthp) *
                  1.0 / (Ntwl * Ntbl);
  }
  ...
  for (i = nr_tdectreesegments; i > 2; i--) {
    /*dt: this too should alternate...*/
    Ceq = (Ctdectreesegments[i - 2] +
           draincap(2 * WtdecdrivetreeN[i - 1], PCH, 1) +
           draincap(WtdecdrivetreeN[i - 1], NCH, 1) +
           gatecap(3 * WtdecdrivetreeN[i - 2], 0.0));
    Req =
        (Rtdectreesegments[i - 2] + transreson(WtdecdrivetreeN[i - 1], NCH, 1));
    tf = Req * Ceq;
    /*dt: This shouldn't be all falling, but interleaved. Have to fix that at
     * some point.*/
    this_delay = horowitz(inrisetime, tf, VTHINV360x240, VTHINV360x240, RISE);
    *Tdecdrive += this_delay;
    ...
    dynPower += tag_bits_routed * Ceq * .5 * VddPow * VddPow;
    lkgCurrent +=
        1.0 / (Ntwl * Ntbl) * pow(2, nr_tdectreesegments - i) *
        tag_bits_routed * 0.5 *
        cmos_ileakage(WtdecdrivetreeN[i - 1], 2 * WtdecdrivetreeN[i - 1],
                      Vt_bit_nmos_low, Vthn, Vt_bit_pmos_low, Vthp);
  }

  if (nr_tdectreesegments) {
    // v4.1: Change below, gatecap(Wtdec3to8n+Wdec3to8p,10.0) ->
    // gatecap(Wtdec3to8n+Wtdec3to8p,10.0) Ceq =
    // 4*gatecap(Wtdec3to8n+Wdec3to8p,10.0) + Ctdectreesegments[0] +
    // draincap(2*WtdecdrivetreeN[0],PCH,1)+ draincap(WtdecdrivetreeN[0],NCH,1);
    Ceq = 4 * gatecap(Wtdec3to8n + Wtdec3to8p, 10.0 / FUDGEFACTOR) +
          Ctdectreesegments[0] + draincap(2 * WtdecdrivetreeN[0], PCH, 1) +
          draincap(WtdecdrivetreeN[0], NCH, 1);
    Rwire = Rtdectreesegments[0];
    tf = (Rwire + transreson(2 * WtdecdrivetreeN[0], PCH, 1)) * Ceq;

    dynPower += tag_bits_routed * Ceq * .5 * VddPow * VddPow;
    lkgCurrent += 1.0 / (Ntwl * Ntbl) * pow(2, nr_tdectreesegments) *
                  tag_bits_routed * 0.5 *
                  cmos_ileakage(WtdecdrivetreeN[0], 2 * WtdecdrivetreeN[0],
                                Vt_bit_nmos_low, Vthn, Vt_bit_pmos_low, Vthp);
  } else {
    // v4.1: Change below, gatecap(Wtdec3to8n+Wdec3to8p,10.0) ->
    // gatecap(Wtdec3to8n+Wtdec3to8p,10.0) Ceq =
    ...
    Ceq = 4 * gatecap(Wtdec3to8n + Wtdec3to8p, 10.0 / FUDGEFACTOR) +
          Ctdectreesegments[0] + draincap(Wtdecdrivep_second, PCH, 1) +
          draincap(Wtdecdriven_second, NCH, 1);
    Rwire = Rtdectreesegments[0];
    tf = (Rwire + transreson(Wtdecdrivep_second, PCH, 1)) * Ceq;

    dynPower += tag_bits_routed * Ceq * .5 * VddPow * VddPow;
    lkgCurrent += 1.0 / (Ntwl * Ntbl) * tag_bits_routed * 0.5 *
                  cmos_ileakage(Wtdecdriven_second, Wtdecdrivep_second,
                                Vt_bit_nmos_low, Vthn, Vt_bit_pmos_low, Vthp);
  }
  this_delay = horowitz(inrisetime, tf, VTHINV360x240, VTHNAND60x120, FALL);
  *Tdecdrive += this_delay;
  ...
  lkgCurrent +=
      8 * 0.2 * 0.5 *
      cmos_ileakage(Wtdec3to8n, Wtdec3to8p, Vt_bit_nmos_low, Vthn,
                    Vt_bit_pmos_low, Vthp) *
      // For the all the 3-8 decoders per quad:
      ceil((1.0 / 3.0) *
           logtwo((double)((double)C / (double)(B * A * Ntbl * Ntspd))))
  ...
  lkgCurrent += 0.5 * 0.2 * rows *
                cmos_ileakage(WtdecNORn, WtdecNORp, Vt_bit_nmos_low, Vthn,
                              Vt_bit_pmos_low, Vthp);
  ...
  dynPower +=
      0.5 * Nact * Ceq * VddPow * VddPow * 4 *
      ceil((1.0 / 3.0) *
           logtwo((double)((double)C / (double)(B * A * Ntbl * Ntspd))));
  // dynPower+=Ntwl*Ntbl*Ceq*VddPow*VddPow*4*ceil((1.0/3.0)*logtwo(
  // (double)((double)C/(double)(B*A*Ntbl*Ntspd))));
  ...
  *Tdecoder1 = horowitz(nextinputtime, tf, VTHNAND60x120, vth, RISE);
  ...
  Ceq = (gatecap(Wtdecinvn + Wtdecinvp, 20.0 / FUDGEFACTOR) +
         numstack * draincap(WtdecNORn, NCH, 1) +
  ...
  *nor_inputs = numstack;
  ...
  lkgCurrent += 0.5 * rows *
                cmos_ileakage(Wtdecinvn, Wtdecinvp, Vt_bit_nmos_low, Vthn,
                              Vt_bit_pmos_low, Vthp);
...
double fa_tag_delay(int C, int B, int Ntwl, int Ntbl, int Ntspd,
                    double *Tagdrive, double *Tag1, double *Tag2, double *Tag3,
                    double *Tag4, double *Tag5, double *outrisetime,
                    int *nor_inputs, powerDef *power) {
  ...
  if (Ntbl > 1) {
    ...
    if (Ntbl <= 4) {
      Ceq = draincap(Wfadrive2p, PCH, 1) + draincap(Wfadrive2n, NCH, 1) +
            gatecap(Wfadecdrive1p + Wfadecdrive1n, 10.0 / FUDGEFACTOR) * 2 +
      ...
      Ceq = draincap(Wfadrive2p, PCH, 1) + draincap(Wfadrive2n, NCH, 1) +
            gatecap(Wfadrivep + Wfadriven, 10.0 / FUDGEFACTOR) * 2 +
            +FACwordmetal * sqrt((rows + 1) * Ntbl) / 2 +
            FACbitmetal * sqrt((rows + 1) * Ntbl) / 2;
      Rwire = FARwordmetal * sqrt((rows + 1) * Ntbl) * .5 / 2 +
              FARbitmetal * sqrt((rows + 1) * Ntbl) * .5 / 2;
  ...
  Ceq = draincap(Wfadecdrivep, PCH, 2) + draincap(Wfadecdriven, NCH, 2) +
        ...
        gatecap(Wdummyn, 10.0 / FUDGEFACTOR) * (rows + 1) +
        FACbitmetal * (rows + 1);

  Rwire = FARbitmetal * (rows + 1) * .5;
  tf = (Rwire + transreson(Wfadecdrivep, PCH, 1) +
        transresswitch(Wfadecdrivep, PCH, 1)) *
       Ceq;
  Tagdrive2 = horowitz(nextinputtime, tf, VTHFA1, VTHFA2, RISE);
  nextinputtime = Tagdrive2 / (1 - VTHFA2);

  *Tagdrive = Tagdrive1 + Tagdrive2 + TagdriveA + TagdriveB + TagdriveA1 +
              TagdriveA2 + TagdriveB1 + TagdriveB2 + Tagdrive0a + Tagdrive0b;
  ...
  Ceq = .5 * ADDRESS_BITS * 2 * draincap(Wdummyn, NCH, 2) +
        ...
        gatecap(Waddrnandn + Waddrnandp, 10.0 / FUDGEFACTOR) +
        FACwordmetal * ADDRESS_BITS;
  Rwire = FARwordmetal * ADDRESS_BITS * .5;
  tf =
      Ceq * (Rwire + transreson(Wdummyn, NCH, 1) + transreson(Wdummyn, NCH, 1));
  *Tag1 = horowitz(nextinputtime, tf, VTHFA2, VTHFA3, FALL);
  ...
  Ceq = draincap(Waddrnandn, NCH, 2) + 2 * draincap(Waddrnandp, PCH, 1) +
  ...
  *Tag2 = horowitz(nextinputtime, tf, VTHFA3, VTHFA4, RISE);
  ...
  Ceq = (rows) * (gatecap(Wfanorn + Wfanorp, 10.0 / FUDGEFACTOR)) +
        draincap(Wdummyinvn, NCH, 1) + draincap(Wdummyinvp, PCH, 1) +
        FACbitmetal * rows;
  ...
  *outrisetime = *Tag3 / VTHFA5;
  ...
  Ceq = draincap(Wfanorp, PCH, 2) + 2 * draincap(Wfanorn, NCH, 1) +
        gatecap(Wfainvn + Wfainvp, 10.0 / FUDGEFACTOR);
  ...
  *outrisetime = *Tag5 / VSINV;
  ...
  //        %f\n 5th - %f %f\nPD : %f\nNAND : %f\n INV : %f\n NOR : %f\n INV :
  //        %f\n", TagdriveA*1e9, TagdriveB*1e9, TagdriveA1*1e9, TagdriveB1*1e9,
  //        TagdriveA2*1e9, TagdriveB2*1e9,
  //        Tagdrive0a*1e9,Tagdrive0b*1e9,Tagdrive1*1e9, Tagdrive2*1e9,
  //        *Tag1*1e9, *Tag2*1e9, *Tag3*1e9, *Tag4*1e9, *Tag5*1e9);
  power->writeOp.dynamic = dynPower;
  power->readOp.dynamic = dynPower;
...
double wordline_delay(int C, int B, int A, int Ndwl, int Ndbl, double Nspd,
  ...
  // v4.1: Fixing double->int type conversion problems. EPSILON is added below
  // to make sure the final int value is the correct one
  ...
  lkgCurrent = 0.5 * cmos_ileakage(WwlDrvn, WwlDrvp, Vt_bit_nmos_low, Vthn,
                                   Vt_bit_pmos_low, Vthp);
  ...
  *outrisetime = Twordchargedel / VSINV;
  ...
  // leakage power for the wordline driver to be accounted in the decoder
  // module..
...
/*dt: incorporated leakage calc from eCacti*/
double wordline_tag_delay(int C, int B, int A, int Ntspd, int Ntwl, int Ntbl,
  ...
  Wtdecinvn = Cload * SizingRatio / 3;
  Wtdecinvp = 2 * Cload * SizingRatio / 3;
  ...
  lkgCurrent = 0.5 * cmos_ileakage(WtwlDrvn, WtwlDrvp, Vt_bit_nmos_low, Vthn,
                                   Vt_bit_pmos_low, Vthp);
  ...
  *outrisetime = Twordchargedel / VSINV;
  ...
  // leakage power for the wordline drivers to be accounted in the decoder
  // module..
  return (Tworddrivedel + Twordchargedel);
}
...
double bitline_delay(int C, int A, int B, int Ndwl, int Ndbl, double Nspd,
  ...
  Iport = cmos_ileakage(Wmemcella, 0, Vt_cell_nmos_high, Vthn,
                        Vt_cell_pmos_high, Vthp);
  Icell = cmos_ileakage(Wmemcellnmos, Wmemcellpmos, Vt_cell_nmos_high, Vthn,
                        Vt_cell_pmos_high, Vthp);
  ...
  // v4.1: Fixing double->int type conversion problems. EPSILON is added below
  // to make sure the final int value is the correct one rows = C / (B * A *
  // Ndbl * Nspd); cols = CHUNKSIZE * B * A * Nspd / Ndwl;
  rows = (int)(C / (B * A * Ndbl * Nspd) + EPSILON);
  cols = (int)(CHUNKSIZE * B * A * Nspd / Ndwl + EPSILON);
  ...
  // Isolation transistor width is set to 10 (typical). Its usually wide to
  // reduce resistance offered for transfer of bitline voltage to sense-amp.
  ...
  // ToDo: calculate the width of sense-amplifier as a function of access
  // times...

  WsPch = 6.25 *
          FEATURESIZE; // ToDo: Determine widths of these devices analytically;
                       // was 5 micron for the 0.8 micron process
  WsenseN = 3.75 * FEATURESIZE; // sense amplifier N-trans; was 3 micron for the
                                // 0.8 micron process
  WsenseP = 7.5 * FEATURESIZE;  // sense amplifier P-trans; was 6 micron for the
                                // 0.8 micron process
  WsenseEn = 5 * FEATURESIZE; // Sense enable transistor of the sense amplifier;
                              // was 4 micron for the 0.8 micron process
  WoBufP =
      10 * FEATURESIZE; // output buffer corresponding to the sense amplifier;
                        // was 8 micron for the 0.8 micron process
  ...
  if (8 * B / BITOUT == 1 && Ndbl * Nspd == 1) {
    ...
    Cline = rows * (Cbitrow + Cbitmetal) + 2 * draincap(Wbitpreequ, PCH, 1) +
            draincap(Wiso, PCH, 1);
    Ccolmux = muxway * (draincap(Wiso, PCH, 1)) +
              gatecap(WsenseN + WsenseP, 10.0 / FUDGEFACTOR);
  ...
  /*dt: If a wordline segment is shorter than a set, then multiple segments have
   * to be activated */
  if (Ndwl < Nspd) {
    Nact = 1;
  } else {
  ...
  now: we only have limited bitline swing thanks to pulsed wordlines. From
  looking at papers from industry I'm assuming that bitline swing can be limited
  to Vbitswing 100mV */
  dynRdEnergy += c2 * VddPow * VbitprePow * cols * Nact;

  /*dt: assuming full swing on the bitlines for writes, but only BITOUT bitlines
   * are activated.*/
  dynWrtEnergy += c2 * VddPow * VddPow * BITOUT;

  // fprintf(stderr, "Pow %f %f\n", c1*VddPow*VddPow*BITOUT*A*muxover*1e9,
  // c2*VddPow*VbitprePow*cols*1e9);
...
double bitline_tag_delay(int C, int A, int B, int Ntwl, int Ntbl, int Ntspd,
  ...
  lkgPort = cmos_ileakage(Wmemcella, 0, Vt_cell_nmos_high, Vthn,
                          Vt_cell_pmos_high, Vthp);
  lkgCell = cmos_ileakage(Wmemcellnmos, Wmemcellpmos, Vt_cell_nmos_high, Vthn,
                          Vt_cell_pmos_high, Vthp);

  if (!force_tag) {
    // v4.1: Fixing double->int type conversion problems. EPSILON is added below
    // to make sure the final int value is the correct one tagbits =
    // ADDRESS_BITS + EXTRA_TAG_BITS - (int) logtwo ((double) C) + (int) logtwo
    // ((double) A) - (int) (logtwo (NSubbanks));
    tagbits =
        (int)(ADDRESS_BITS + EXTRA_TAG_BITS - (int)logtwo((double)C) +
              (int)logtwo((double)A) - (int)(logtwo(NSubbanks)) + EPSILON);
  } else {
  ...
  WtsPch = 6.25 *
           FEATURESIZE; // ToDo: Determine widths of these devices analytically;
                        // was 5 micron for the 0.8 micron process
  WtsenseN = 3.75 * FEATURESIZE; // sense amplifier N-trans; was 3 micron for
                                 // the 0.8 micron process
  WtsenseP = 7.5 * FEATURESIZE; // sense amplifier P-trans; was 6 micron for the
                                // 0.8 micron process
  WtsenseEn =
      5 * FEATURESIZE; // Sense enable transistor of the sense amplifier; was 4
  ...
  // v4.1: Ntspd*Ntbl in the colmux of the else condition does not make sense
  // and overestimates tag bitline delay by a huge amount when Ntspd and Ndbl
  // are large. Fixing this and fixing the if condition as well
  /*if (Ntbl * Ntspd == 1)
    {
  ...
  else
    {
      Cline = rows*(Cbitrow+TagCbitmetal) + 2*draincap(Wtbitpreequ,PCH,1) +
       ...
       Ccolmux =
  Ntspd*Ntbl*(draincap(Wtiso,PCH,1))+gatecap(WtsenseN+WtsenseP,10.0
  /FUDGEFACTOR); Rlineb = TagRbitmetal*rows/2.0; r1 = Rlineb +
  transreson(Wtiso,PCH,1);
    }*/

  if (Ntspd / Ntwl <
      1) { // Ntspd/Ntwl = Tag subarray columns / Tag array output width =
    //= (A * tagbits * Ntspd / Ntwl) / A * tagbits = Degree of column muxing in
    //a tag subarray. When it's less than 1, it means that more than one tag
    // subarray is required to produce the tag array output width.
    Cline = rows * (Cbitrow + TagCbitmetal) + 2 * draincap(Wtbitpreequ, PCH, 1);
    Ccolmux = gatecap(WtsenseN + WtsenseP, 10.0 / FUDGEFACTOR);
    Rlineb = TagRbitmetal * rows / 2.0;
    r1 = Rlineb;
  } else { // Tag array output width comes from one tag subarray
    Cline = rows * (Cbitrow + TagCbitmetal) +
            2 * draincap(Wtbitpreequ, PCH, 1) + draincap(Wtiso, PCH, 1);
    Ccolmux = Ntspd / Ntwl * (draincap(Wtiso, PCH, 1)) +
              gatecap(WtsenseN + WtsenseP, 10.0 / FUDGEFACTOR);
    Rlineb = TagRbitmetal * rows / 2.0;
  ...
  c1 = Ccolmux;
  c2 = Cline;
  // v4.1: The tag energy calculation did not make use of the computed Nact
  // below. Fixing that.
...
double sense_amp_delay(int C, int B, int A, int Ndwl, int Ndbl, double Nspd,
  ...
  if (Ndwl / Nspd < 1) {
    ...
    // v4.1: Fixing double->int type conversion problems. EPSILON is added below
    // to make sure the final int value is the correct one Nact = Ndwl/Nspd;
    Nact = (int)(Ndwl / Nspd + EPSILON);
  }
  ...
  // sense amplifier has to drive logic in "data out driver" and sense precharge
  // load. load seen by sense amp

  Cload = gatecap(WsenseP + WsenseN, 5.0 / FUDGEFACTOR) +
          draincap(WsPch, PCH, 1) +
          gatecap(Woutdrvnandn + Woutdrvnandp, 1.0 / FUDGEFACTOR) +
          gatecap(Woutdrvnorn + Woutdrvnorp, 1.0 / FUDGEFACTOR);
  ...
  *outrisetime = tfalldata;
  ...
  return (tsensedata +
          2.5e-10 / FUDGEFACTOR); // v4.1: Computing sense amp delay component
                                  // for input tech node itself instead
...
double sense_amp_tag_delay(int C, int B, int A, int Ntwl, int Ntbl, int Ntspd,
  ...
  if (!force_tag) {
    // v4.1: Fixing double->int type conversion problems. EPSILON is added below
    // to make sure the final int value is the correct one tagbits =
    // ADDRESS_BITS + EXTRA_TAG_BITS
    // -(int)logtwo((double)C)+(int)logtwo((double)A)-(int)(logtwo(NSubbanks));
    tagbits =
        (int)(ADDRESS_BITS + EXTRA_TAG_BITS - (int)logtwo((double)C) +
              (int)logtwo((double)A) - (int)(logtwo(NSubbanks)) + EPSILON);
  } else {
  ...
  Cload = gatecap(WtsenseP + WtsenseN, 5.0 / FUDGEFACTOR) +
          draincap(WtsPch, PCH, 1);
  // gatecap(Woutdrvnandn+Woutdrvnandp,1.0) +
  // gatecap(Woutdrvnorn+Woutdrvnorp,1.0);
...
double compare_time(int C, int A, int Ntbl, int Ntspd, double NSubbanks,
  ...
  lkgCurrent = 0.5 * A *
               cmos_ileakage(Wcompinvn1, Wcompinvp1, Vt_bit_nmos_low, Vthn,
                             Vt_bit_pmos_low, Vthp);
  /* Second Inverter */
  ...
  lkgCurrent += 0.5 * A *
                cmos_ileakage(Wcompinvn2, Wcompinvp2, Vt_bit_nmos_low, Vthn,
                              Vt_bit_pmos_low, Vthp);
  ...
  lkgCurrent += 0.5 * A *
                cmos_ileakage(Wcompinvn3, Wcompinvp3, Vt_bit_nmos_low, Vthn,
                              Vt_bit_pmos_low, Vthp);
  ...
  c1 = (tagbits) * (draincap(Wcompn, NCH, 1) + draincap(Wcompn, NCH, 2)) +
       ...
       gatecap(WmuxdrvNANDn + WmuxdrvNANDp, 20.0 / FUDGEFACTOR) +
       cols * Cwordmetal;
  dynPower += c2 * VddPow * VddPow * 2 * A;
  dynPower += c1 * VddPow * VddPow * (A - 1);

  lkgCurrent += 0.5 * A *
                cmos_ileakage(Wevalinvn, Wevalinvp, Vt_bit_nmos_low, Vthn,
                              Vt_bit_pmos_low, Vthp);
  ...
  lkgCurrent += 2 * cols * 0.2 * 0.5 * A *
                cmos_ileakage(Wcompn, Wcompp, Vt_bit_nmos_low, Vthn,
                              Vt_bit_pmos_low, Vthp);
  // leakage due to the mux driver
  lkgCurrent += 0.5 * A *
                cmos_ileakage(Wmuxdrv12n, Wmuxdrv12p, Vt_bit_nmos_low, Vthn,
                              Vt_bit_pmos_low, Vthp);
  ...
  if ((tstep) <= (0.5 * (Vdd - Vt) / m)) {
    ...
    c = -2 * (tstep) * (Vdd - Vt) +
        1 / m * ((Vdd * VTHEVALINV) - Vt) * ((Vdd * VTHEVALINV) - Vt);
...
double mux_driver_delay(int Ntbl, int Ntspd, double inputtime,
  ...
  Ceq = gatecap(Wmuxdrv3n + Wmuxdrv3p, 15.0 / FUDGEFACTOR) +
        2 * draincap(WmuxdrvNORn, NCH, 1) + draincap(WmuxdrvNORp, PCH, 2);
  ...
  Ceq = BITOUT * gatecap(Woutdrvseln + Woutdrvselp + Woutdrvnorn + Woutdrvnorp,
                         20.0) +
...
void precalc_muxdrv_widths(int C, int B, int A, int Ndwl, int Ndbl, double Nspd,
  ...
  int l_muxdrv_v = 0, l_muxdrv_h = 0, cols_subarray, rows_subarray,
      nr_subarrays_left, horizontal_step, vertical_step, v_or_h;
  ...
  // v4.1: Fixing double->int type conversion problems. EPSILON is added below
  // to make sure the final int value is the correct one cols_subarray =
  // (CHUNKSIZE * B * A * Nspd / Ndwl); rows_subarray = (C /(B * A * Ndbl *
  // Nspd));
  ...
  if (Ndwl * Ndbl == 1) {
    ...
    Cline =
        BITOUT * gatecap(Woutdrvseln + Woutdrvselp + Woutdrvnorn + Woutdrvnorp,
                         20.0 / FUDGEFACTOR) +
    ...
    while (nr_subarrays_left > 1) {
      ...
      if (v_or_h) {
        ...
        Cmuxdrvtreesegments[nr_muxdrvtreesegments - 1] =
            GlobalCbitmetal * vertical_step;
        Rmuxdrvtreesegments[nr_muxdrvtreesegments - 1] =
            0.5 * GlobalRbitmetal * vertical_step;
        Cline = gatecap(previous_ndriveW + previous_pdriveW, 0) +
                GlobalCbitmetal * vertical_step;
  ...
  Ceq = BITOUT * gatecap(Woutdrvseln + Woutdrvselp + Woutdrvnorn + Woutdrvnorp,
                         20.0 / FUDGEFACTOR);
  /*dt: adding an extra layer of drivers, so  that we don't have these biiiig
   * gates */
  ...
  WmuxdrvNANDn =
      2 * (WmuxdrvNORn + WmuxdrvNORp) * muxover * SizingRatio * 1 / 3;
...
double mux_driver_delay_dualin(int C, int B, int A, int Ntbl, int Ntspd,
                               double inputtime1, double *outputtime,
                               double wirelength_v, double wirelength_h,
                               powerDef *power) {
  ...
  lkgCurrent = 0.5 * 0.2 * muxover *
               cmos_ileakage(WmuxdrvNORn, WmuxdrvNORp, Vt_bit_nmos_low, Vthn,
                             Vt_bit_pmos_low, Vthp);
  lkgCurrent += 0.5 * 0.2 * muxover *
                cmos_ileakage(WmuxdrvNANDn, WmuxdrvNANDp, Vt_bit_nmos_low, Vthn,
                              Vt_bit_pmos_low, Vthp);
  ...
  lkgCurrent += 0.5 * muxover *
                cmos_ileakage(Wmuxdrv3n, Wmuxdrv3p, Vt_bit_nmos_low, Vthn,
                              Vt_bit_pmos_low, Vthp);
  ...
  if (nr_muxdrvtreesegments) {
    Ceq = draincap(Wmuxdrv3p, PCH, 1) + draincap(Wmuxdrv3n, NCH, 1) +
          gatecap(3 * WmuxdrvtreeN[nr_muxdrvtreesegments - 1], 0) +
          Cmuxdrvtreesegments[nr_muxdrvtreesegments - 1];
    Req = transreson(Wmuxdrv3n, NCH, 1) +
          Rmuxdrvtreesegments[nr_muxdrvtreesegments - 1];
  ...
  for (i = nr_muxdrvtreesegments; i > 2; i--) {
    /*dt: this too should alternate...*/
    Ceq = (Cmuxdrvtreesegments[i - 2] +
           draincap(2 * WmuxdrvtreeN[i - 1], PCH, 1) +
           draincap(WmuxdrvtreeN[i - 1], NCH, 1) +
           gatecap(3 * WmuxdrvtreeN[i - 2], 0.0));
    Req =
        (Rmuxdrvtreesegments[i - 2] + transreson(WmuxdrvtreeN[i - 1], NCH, 1));
    tf = Req * Ceq;
    /*dt: This shouldn't be all falling, but interleaved. Have to fix that at
     * some point.*/
    this_delay = horowitz(inrisetime, tf, VTHINV360x240, VTHINV360x240, RISE);
    ...
    lkgCurrent += pow(2, nr_dectreesegments - i) * A * 0.5 *
                  cmos_ileakage(WmuxdrvtreeN[i - 1], 2 * WmuxdrvtreeN[i - 1],
                                Vt_bit_nmos_low, Vthn, Vt_bit_pmos_low, Vthp);
  }
  ...
  if (nr_muxdrvtreesegments) {
    Ceq =
        BITOUT * gatecap(Woutdrvseln + Woutdrvselp + Woutdrvnorn + Woutdrvnorp,
                         20.0 / FUDGEFACTOR) +
        +Cdectreesegments[0] + draincap(2 * WmuxdrvtreeN[0], PCH, 1) +
        draincap(WmuxdrvtreeN[0], NCH, 1);
    ...
    dynPower += A * Ceq * .5 * VddPow * VddPow;
    lkgCurrent += pow(2, nr_muxdrvtreesegments) * A * 0.5 *
                  cmos_ileakage(WmuxdrvtreeN[0], 2 * WmuxdrvtreeN[0],
                                Vt_bit_nmos_low, Vthn, Vt_bit_pmos_low, Vthp);
    ...
    Req = (GlobalRwordmetal * wirelength_h + GlobalRbitmetal * wirelength_v) /
          2.0;
...
double senseext_driver_delay(int A, char fullyassoc, double inputtime,
                             double *outputtime, double wirelength_sense_v,
                             double wirelength_sense_h, powerDef *power) {
  double Ceq, Req, tf, nextinputtime;
  // double Tst1,Tst2,Tst3;
  double Tst1, Tst2;
  double lkgCurrent = 0.0, dynPower = 0.0;
  ...
  lkgCurrent = 0.5 * BITOUT * A * muxover *
               cmos_ileakage(Wsenseextdrv1n, Wsenseextdrv1p, Vt_bit_nmos_low,
                             Vthn, Vt_bit_pmos_low, Vthp);
  /* second driver stage */

  Ceq = draincap(Wsenseextdrv2p, PCH, 1) + draincap(Wsenseextdrv2n, NCH, 1) +
        GlobalCwordmetal * wirelength_sense_h +
        GlobalCbitmetal * wirelength_sense_v;

  Req = (GlobalRwordmetal * wirelength_sense_h +
         GlobalRbitmetal * wirelength_sense_v) /
            2.0 +
        transreson(Wsenseextdrv2p, PCH, 1);
  ...
  *outputtime = Tst2 / (1 - VTHOUTDRNAND);
  ...
  lkgCurrent = 0.5 * BITOUT * A * muxover *
               cmos_ileakage(Wsenseextdrv2n, Wsenseextdrv2p, Vt_bit_nmos_low,
                             Vthn, Vt_bit_pmos_low, Vthp);
  //   fprintf(stderr, "Pow %f %f\n", Ceq*VddPow*VddPow*.5*BITOUT*A*muxover*1e3,
  //   Ceq*VddPow*VddPow*.5*BITOUT*A*muxover*1e3);
...
double total_out_driver_delay(int C, int B, int A, char fullyassoc, int Ndbl,
                              double Nspd, int Ndwl, int Ntbl, int Ntwl,
                              int Ntspd, double NSubbanks, double inputtime,
                              double *outputtime, powerDef *power) {
  ...
  if (!fullyassoc) {
    ...
    if (!force_tag) {
      ...
      // the final int value is the correct one
      // tagbits = ADDRESS_BITS + EXTRA_TAG_BITS - (int) logtwo ((double) B);
      tagbits = (int)(ADDRESS_BITS + EXTRA_TAG_BITS - (int)logtwo((double)B) +
                      EPSILON);
    } else {
      tagbits = force_tag_size;
    }
...
double valid_driver_delay(int C, int B, int A, char fullyassoc, int Ndbl,
                          int Ndwl, double Nspd, int Ntbl, int Ntwl, int Ntspd,
                          double *NSubbanks, double inputtime,
                          powerDef *power) {
  double Ceq, Tst1, tf;
  ...
  wire_cap = GlobalCbitmetal * (l_valdrv_v + subbank_v) +
             GlobalCwordmetal * (l_valdrv_h + subbank_h);
  wire_res = GlobalRwordmetal * (l_valdrv_h + subbank_h) * 0.5 +
             GlobalRbitmetal * (l_valdrv_v + subbank_v) * 0.5;

  Ceq = draincap(Wmuxdrv12n, NCH, 1) + draincap(Wmuxdrv12p, PCH, 1) + wire_cap +
        Cout;
...
double half_compare_delay(int C, int B, int A, int Ntwl, int Ntbl, int Ntspd,
  ...
  int cols, tagbits;
  double lkgCurrent = 0.0, dynPower = 0.0;
  int v_or_h = 0, hori = 0, vert = 0, horizontal_step = 0, vertical_step = 0,
      nr_subarrays_left = 0;
  double Wxtramuxdrv1n = 0, Wxtramuxdrv1p = 0, Wxtramuxdrv2n = 0,
         Wxtramuxdrv2p = 0;
  double Cload = 0;
  ...
  /*dt: Certainly don't need the dirty bit in comparison. The valid bit is
   * assumed to be handled elsewhere, because we don't need to route any data to
   * compare with to it.*/
  if (!force_tag) {
    // v4.1: Fixing double->int type conversion problems. EPSILON is added below
    // to make sure
    // the final int value is the correct one
    // tagbits = ADDRESS_BITS - (int)logtwo((double)C) +
    // (int)logtwo((double)A)-(int)(logtwo(NSubbanks));
    tagbits =
        (int)(ADDRESS_BITS - (int)logtwo((double)C) + (int)logtwo((double)A) -
              (int)(logtwo(NSubbanks)) + EPSILON);
    /*dt: we have to round up in case we have an odd number of bits in the tag,
     * to account for the longer path */
    if ((tagbits % 4) == 0) {
      tagbits = tagbits / 4;
    } else {
      tagbits = tagbits / 4 + 1;
  ...
  /*dt: I think we need to add at least one buffer (probably more) between the
   * comparator output gate and the mux driver ('cause it's biig!)*/
  ...
  lkgCurrent = 0.5 * A *
               cmos_ileakage(Wxtramuxdrv2n, Wxtramuxdrv2p, Vt_bit_nmos_low,
                             Vthn, Vt_bit_pmos_low, Vthp);
  ...
  *outputtime = muxdrv1del / VTHCOMPINV;
  ...
  lkgCurrent = 0.5 * A *
               cmos_ileakage(Wxtramuxdrv1n, Wxtramuxdrv1p, Vt_bit_nmos_low,
                             Vthn, Vt_bit_pmos_low, Vthp);
...
double dataoutput_delay(int C, int B, int A, char fullyassoc, int Ndbl,
                        double Nspd, int Ndwl, double inrisetime,
                        double *outrisetime, powerDef *power) {
  ...
  double this_delay;
  int i;
  /*dt: if we have fast cache mode, all A ways are routed to the edge of the
   * data array*/
  ...
  lkgCurrent += 0.5 * BITOUT * routed_ways * muxover *
                cmos_ileakage(Woutdrvseln, Woutdrvselp, Vt_bit_nmos_low, Vthn,
                              Vt_bit_pmos_low, Vthp);
  lkgCurrent += 0.5 * 0.2 * BITOUT * routed_ways * muxover *
                cmos_ileakage(Woutdrvnorn, Woutdrvnorp, Vt_bit_nmos_low, Vthn,
                              Vt_bit_pmos_low, Vthp);
  lkgCurrent += 0.5 * 0.2 * BITOUT * routed_ways * muxover *
                cmos_ileakage(Woutdrvnandn, Woutdrvnandp, Vt_bit_nmos_low, Vthn,
                              Vt_bit_pmos_low, Vthp);

  /* Delay of output driver tree*/

  if (nr_outdrvtreesegments) {
    Ceq = (draincap(Woutdrivern, NCH, 1) + draincap(Woutdriverp, PCH, 1)) * A *
              muxover +
    ...
    Ceq = (draincap(Woutdrivern, NCH, 1) + draincap(Woutdriverp, PCH, 1)) * A *
              muxover +
          Coutdrvtreesegments[0] +
          gatecap(Wsenseextdrv1n + Wsenseextdrv1p, 10.0 / FUDGEFACTOR);
  }
  Rwire = Routdrvtreesegments[0];

  dynPower += routed_ways * Ceq * VddPow * VddPow * .5 *
              BITOUT; // factor of routed_ways added by Shyam

  lkgCurrent += 0.5 * BITOUT * routed_ways * muxover *
                cmos_ileakage(Woutdrivern, Woutdriverp, Vt_bit_nmos_low, Vthn,
                              Vt_bit_pmos_low, Vthp);
  ...
  *outrisetime = outdel / (1.0 - VTHOUTDRIVE);
  ...
  if (nr_outdrvtreesegments) {
    for (i = 1; i < nr_outdrvtreesegments - 1; i++) {
      /*dt: this too should alternate...*/
      Ceq = (Coutdrvtreesegments[i] + draincap(2 * WoutdrvtreeN[i], PCH, 1) +
             draincap(WoutdrvtreeN[i], NCH, 1) +
       ...
       * some point.*/
      this_delay =
          horowitz(*outrisetime, tf, VTHINV360x240, VTHINV360x240, RISE);
      outdel += this_delay;
      *outrisetime = this_delay / (1.0 - VTHINV360x240);

      dynPower += routed_ways * BITOUT * Ceq * .5 * VddPow * VddPow;
      lkgCurrent += pow(2, nr_outdrvtreesegments - i) * routed_ways * BITOUT *
                    0.5 *
                    cmos_ileakage(WoutdrvtreeN[i], 2 * WoutdrvtreeN[i],
                                  Vt_bit_nmos_low, Vthn, Vt_bit_pmos_low, Vthp);
    }
    Ceq = Coutdrvtreesegments[nr_outdrvtreesegments - 1] +
          draincap(2 * WoutdrvtreeN[nr_outdrvtreesegments - 1], PCH, 1) +
          draincap(WoutdrvtreeN[nr_outdrvtreesegments - 1], NCH, 1) +
          gatecap(Wsenseextdrv1n + Wsenseextdrv1p, 10.0 / FUDGEFACTOR);
    Req = (Routdrvtreesegments[nr_outdrvtreesegments - 1] +
           transreson(WoutdrvtreeN[nr_outdrvtreesegments - 1], NCH, 1));
    tf = Req * Ceq;
    /*dt: This shouldn't be all falling, but interleaved. Have to fix that at
     * some point.*/
    ...
    *outrisetime = this_delay / (1.0 - VTHINV360x240);
    dynPower += routed_ways * BITOUT * Ceq * .5 * VddPow * VddPow;
    lkgCurrent += routed_ways * BITOUT * 0.5 *
                  cmos_ileakage(WoutdrvtreeN[nr_outdrvtreesegments - 1],
                                2 * WoutdrvtreeN[nr_outdrvtreesegments - 1],
                                Vt_bit_nmos_low, Vthn, Vt_bit_pmos_low, Vthp);
  }
...
void calc_wire_parameters(parameter_type *parameters) {
  ...
  {
    FACbitmetal =
        ((32 / FUDGEFACTOR + 2 * WIREPITCH *
                                 ((parameters->num_write_ports +
                                   parameters->num_readwrite_ports - 1) +
                                  parameters->num_read_ports)) *
         Default_Cmetal);
    FACwordmetal = ((BitWidth +
                     2 * WIREPITCH *
                         ((parameters->num_write_ports +
                           parameters->num_readwrite_ports - 1)) +
                     WIREPITCH * (parameters->num_read_ports)) *
                    Default_Cmetal);
    // v4.1: Fixing an error in the resistance equations below. WIREHEIGHTRATIO
    // is replaced by WIREWIDTH.
    /*FARbitmetal =
      (((32 +
         2 * WIREPITCH *
         ((parameters->
          ...
          parameters->num_read_ports))) *
              (Default_CopperSheetResistancePerMicroM /WIREHEIGTHRATIO *
    FUDGEFACTOR)); FARwordmetal =
      (((BitWidth +
         2 * WIREPITCH *
         ((parameters->
           num_write_ports + parameters->num_readwrite_ports - 1)) +
    ...
    FARwordmetal =
        (((BitWidth +
           2 * WIREPITCH *
               ((parameters->num_write_ports + parameters->num_readwrite_ports -
                 1)) +
           WIREPITCH * (parameters->num_read_ports))) *
         (Default_CopperSheetResistancePerMicroM / WIREWIDTH * FUDGEFACTOR));
  }
  /*dt: Capacitance gets better with scaling, because C = K*epsilon*L*(W/x_ox +
     H/L_s), where W: width of the wire, L: length of the wire, H: height of the
     wire, x_ox: thickness of the oxide separating two wires vertically, L_s:
     thickness of the insulator separating two wires horizontally K and epsilon
     are constants To a first order all the variables scale with featuresize =>
     C~ const. * f^2/f = const. * f Note that FUDGEFACTOR is the inverse of
     featuresize Since we're going to divide by FUDGEFACTOR in the end anyway,
     I'm leaving this alone and just adding this comment for future reference.
*/
  Cbitmetal = ((BitHeight + 2 * WIREPITCH *
                                ((parameters->num_write_ports +
                                  parameters->num_readwrite_ports - 1) +
                                 parameters->num_read_ports)) *
               Default_Cmetal);
  Cwordmetal = ((BitWidth +
                 ...
                 WIREPITCH * (parameters->num_read_ports)) *
                Default_Cmetal);
  /* dt: Resistance gets worse, since R = rho*L/(W*H), where L, W, H are the
     length, width and height of the wire. All scale with featuresize, so R ~
     const*f/f^2 = const./f Note that FUDGEFACTOR is the inverse of featuresize
  */

  // v4.1: Fixing an error in the resistance equations below. WIREHEIGHTRATIO is
  // replaced by
  //  WIREWIDTH.
  /*Rbitmetal =
    (((BitHeight +
  ...
  parameters->num_read_ports))) * (Default_CopperSheetResistancePerMicroM
  /WIREHEIGTHRATIO * FUDGEFACTOR)); Rwordmetal =
    (((BitWidth +
       2 * WIREPITCH *
       ((parameters->num_write_ports + parameters->num_readwrite_ports - 1)) +
  WIREPITCH * (parameters->num_read_ports))) *
  ...
  /*dt: tags only need one RW port for when a line is evicted. All other ports
   * can be simple read ports */
  TagCbitmetal = (BitHeight + 2 * WIREPITCH *
                                  (parameters->num_read_ports +
                                   parameters->num_readwrite_ports - 1)) *
                 Default_Cmetal;
  /*dt: the +1 is because a RW port needs two bitlines, thus longer wordlines */
  TagCwordmetal =
      (BitWidth + WIREPITCH * (parameters->num_read_ports +
                               2 * (parameters->num_readwrite_ports - 1))) *
      Default_Cmetal;
  /*TagRbitmetal = (BitHeight + 2 * WIREPITCH * (parameters->num_read_ports +
  parameters->num_readwrite_ports - 1))  *
                                 (Default_CopperSheetResistancePerMicroM
  /WIREHEIGTHRATIO * FUDGEFACTOR); TagRwordmetal = (BitWidth + WIREPITCH *
  (parameters->num_read_ports + 2*(parameters->num_readwrite_ports - 1))) *
                      (Default_CopperSheetResistancePerMicroM /WIREHEIGTHRATIO *
  FUDGEFACTOR);*/

  TagRbitmetal = (BitHeight + 2 * WIREPITCH *
                                  (parameters->num_read_ports +
                                   parameters->num_readwrite_ports - 1)) *
                 (Default_CopperSheetResistancePerMicroM / WIREHEIGTHRATIO *
...
void calculate_time(result_type *result, arearesult_type *arearesult,
  ...
  double decoder_data_driver = 0.0, decoder_data_3to8 = 0.0,
         decoder_data_inv = 0.0;
  double decoder_data = 0.0, decoder_tag = 0.0, wordline_data = 0.0,
         wordline_tag = 0.0;
  powerDef decoder_data_power, decoder_tag_power, wordline_data_power,
      wordline_tag_power;
  double decoder_tag_driver = 0.0, decoder_tag_3to8 = 0.0,
         decoder_tag_inv = 0.0;
  double bitline_data = 0.0, bitline_tag = 0.0, sense_amp_data = 0.0,
         sense_amp_tag = 0.0;
  powerDef sense_amp_data_power, sense_amp_tag_power, bitline_tag_power,
      bitline_data_power;
  ...
  double tag_delay_part1 = 0.0, tag_delay_part2 = 0.0, tag_delay_part3 = 0.0,
         tag_delay_part4 = 0.0, tag_delay_part5 = 0.0, tag_delay_part6 = 0.0;
  double max_delay = 0;
  int counter;
  double Tpre, Tpre_tag;
  int allports = 0, allreadports = 0, allwriteports = 0;
  int tag_iteration_counter = 0;
  double colsfa, desiredrisetimefa, Clinefa,
      Rpdrivefa; // Added by Shyam to incorporate FA caches
  // in v4.0
  ...
  /*dt: added to simulate accessing the tags and then only accessing one data
   * way*/
  sequential_access_flag = parameters->sequential_access;
  /*dt: added to simulate fast and power hungry caches which route all the ways
     to the edge of the data array and do the way select there, instead of
     waiting for the way select signal to travel all the way to the subarray.
  */
  ...
  for (counter = 0; counter < 2; counter++) {
    if (!parameters->fully_assoc) {
      ...
      It also requires extra muxes before the sense amplifiers to select the
      right set.
      */
      // v4.1: We now consider fractions of Nspd while exploring the
      // partitioning
      // space. The lower bound of Nspd is BITOUT /(parameters->block_size*8).
      for (Nspd = (double)(BITOUT) / (double)(parameters->block_size * 8);
           Nspd <= MAXDATASPD; Nspd = Nspd * 2) {
        /*
        Varies the number of wordline segments,
        i.e. into how many "sub"-wordlines each wordline is split.
        Increasing the number of segments, increases the number of wordline
        drivers, but makes each segment shorter and faster.
        */
        for (Ndwl = 1; Ndwl <= MAXDATAN; Ndwl = Ndwl * 2) {
          ...
          Increasing the number of segments increases the number of sense
          amplifiers needed, but makes each bitline segment shorter and faster.
          */
          for (Ndbl = 1; Ndbl <= MAXDATAN; Ndbl = Ndbl * 2) {
            ...
            */
            if (data_organizational_parameters_valid(
                    parameters->block_size, parameters->tag_associativity,
                    parameters->cache_size, Ndwl, Ndbl, Nspd,
              ...
              if (8 * parameters->block_size / BITOUT == 1 && Nspd == 1) {
                ...
                if (Nspd > MAX_COL_MUX) {
                  ...
                  if (8 * parameters->block_size * Nspd / BITOUT >
                      MAX_COL_MUX) {
                    muxover = (8 * parameters->block_size / BITOUT) /
                              (MAX_COL_MUX / (Nspd));
                  } else {
              ...
              compute_device_widths(parameters->cache_size,
              ...
              if (*NSubbanks == 1.0) {
                ...
                decoder_data = decoder_delay(
                    parameters->cache_size, parameters->block_size,
                    parameters->data_associativity, Ndwl, Ndbl, Nspd,
                    *NSubbanks, &decoder_data_driver, &decoder_data_3to8,
                    &decoder_data_inv, inrisetime, &outrisetime,
                    &data_nor_inputs, &decoder_data_power);
                ...
                bitline_data = bitline_delay(
                    parameters->cache_size, parameters->data_associativity,
                    parameters->block_size, Ndwl, Ndbl, Nspd, inrisetime,
                    &outrisetime, &bitline_data_power, Tpre);

                inrisetime = outrisetime;
                max_delay = MAX(max_delay, bitline_data);

                reset_powerDef(&sense_amp_data_power);
                sense_amp_data = sense_amp_delay(
                    parameters->cache_size, parameters->block_size,
                    parameters->data_associativity, Ndwl, Ndbl, Nspd,
                    inrisetime, &outrisetime, &sense_amp_data_power);
                ...
                data_output = dataoutput_delay(
                    parameters->cache_size, parameters->block_size,
                    parameters->data_associativity, parameters->fully_assoc,
                    Ndbl, Nspd, Ndwl, inrisetime, &outrisetime,
                    &data_output_power);
         ...
         the data access has been done.
                */
                if (parameters->data_associativity == 1) {
                  before_mux = subbank_address_routing_delay + decoder_data +
              ...
              for (Ntspd = 1; Ntspd <= MAXTAGSPD; Ntspd = Ntspd * 2) {
                ...
                */
                /*dt: Ntwl is no longer limited to one. We assume a wire-or for
                 * the valid and select lines. */
                  ...
                  */
                  for (Ntbl = 1; Ntbl <= MAXTAGN; Ntbl = Ntbl * 2) {
                    ...
                    if (tag_organizational_parameters_valid(
                            parameters->block_size,
                            parameters->tag_associativity,
                            parameters->cache_size, Ntwl, Ntbl, Ntspd,
                            parameters->fully_assoc, (*NSubbanks))) {
                      tag_iteration_counter++;

                      reset_tag_device_widths();

                      compute_tag_device_widths(
                          parameters->cache_size, parameters->block_size,
                          parameters->tag_associativity, Ntspd, Ntwl, Ntbl,
                          (*NSubbanks));

                      area_subbanked(
                          ADDRESS_BITS, BITOUT, parameters->num_readwrite_ports,
                          parameters->num_read_ports,
                          parameters->num_write_ports, Ndbl, Ndwl, Nspd, Ntbl,
                          Ntwl, Ntspd, *NSubbanks, parameters,
                          &arearesult_subbanked_temp, &arearesult_temp);

                      Subbank_Efficiency =
                          (area_all_dataramcells + area_all_tagramcells) * 100 /
                          (arearesult_temp.totalarea / 100000000.0);

                      // v4.1: No longer using calculate_area function as area
                      // has already been computed for the given tech node
                      /*Total_Efficiency =
                        (*NSubbanks) *
                      ...
                      Total_Efficiency =
                          ...
                          (arearesult_subbanked_temp.height *
                           arearesult_subbanked_temp.width / 100000000.0);
                      ...
                      if (*NSubbanks > 1) {
                        ...
                        subbank_dim(
                            ...
                            parameters->fully_assoc, Ndbl, Ndwl, Nspd, Ntbl,
                            Ntwl, Ntspd, *NSubbanks, &bank_h, &bank_v);
                        ...
                        reset_powerDef(&subbank_address_routing_power);
                        /*dt: this has to be reset on every loop iteration, or
                           else we implicitly reuse the risetime from the end of
                           the last loop iteration!*/
                        inrisetime = addr_inrisetime = 0;
                        if (*NSubbanks > 2) {
                          subbank_address_routing_delay = address_routing_delay(
                              parameters->cache_size, parameters->block_size,
                              parameters->data_associativity,
                              parameters->fully_assoc, Ndwl, Ndbl, Nspd, Ntwl,
                              Ntbl, Ntspd, NSubbanks, &outrisetime,
                              &subbank_address_routing_power);

                          /*dt: moved this here, because we only have extra
                             signal slope if there's something before the
                             decoder */
                          inrisetime = outrisetime;
                        ...
                        decoder_data = decoder_delay(
                            parameters->cache_size, parameters->block_size,
                            parameters->data_associativity, Ndwl, Ndbl, Nspd,
                            *NSubbanks, &decoder_data_driver,
                            &decoder_data_3to8, &decoder_data_inv, inrisetime,
                            &outrisetime, &data_nor_inputs,
                            &decoder_data_power);
                        ...
                        inrisetime = outrisetime;
                        max_delay = MAX(max_delay, wordline_data);
                        /*dt: assuming precharge delay is decoder + wordline
                         * delay */
                        Tpre += wordline_data;
                        ...
                        bitline_data =
                            bitline_delay(parameters->cache_size,
                                          parameters->data_associativity,
                                          parameters->block_size, Ndwl, Ndbl,
                                          Nspd, inrisetime, &outrisetime,
                        ...
                        data_output = dataoutput_delay(
                            parameters->cache_size, parameters->block_size,
                            parameters->data_associativity,
                            parameters->fully_assoc, Ndbl, Nspd, Ndwl,
                        ...
                        subbank_routing_length(
                            parameters->cache_size, parameters->block_size,
                            parameters->data_associativity,
                            parameters->fully_assoc, Ndbl, Nspd, Ndwl, Ntbl,
                            Ntwl, Ntspd, *NSubbanks, &subbank_v, &subbank_h);

                        if (*NSubbanks > 2) {
                          total_out_driver = senseext_driver_delay(
                              parameters->data_associativity,
                              parameters->fully_assoc, inrisetime, &outrisetime,
                              subbank_v, subbank_h, &total_out_driver_power);
                        }

                        inrisetime = outrisetime;
                        max_delay = MAX(max_delay, total_out_driver);

                        /* if the associativity is 1, the data output can come
                           right after the sense amp.   Otherwise, it has to
                           wait until the data access has been done. */

                        if (parameters->data_associativity == 1) {
                          before_mux = subbank_address_routing_delay +
                      ...
                      if (tag_iteration_counter < MAX_CACHE_ENTRIES &&
                          valid_tag_cache_entries[tag_iteration_counter] ==
                              TRUE) {
                        current_cache_entry =
                            cached_tag_calculations[tag_iteration_counter];
                        decoder_tag = current_cache_entry.decoder.delay;
                        copy_powerDef(&decoder_tag_power,
                                      current_cache_entry.decoder.power);
                      } else {
                        decoder_tag = decoder_tag_delay(
                            parameters->cache_size, parameters->block_size,
                            parameters->tag_associativity, Ntwl, Ntbl, Ntspd,
                            *NSubbanks, &decoder_tag_driver, &decoder_tag_3to8,
                            &decoder_tag_inv, addr_inrisetime, &outrisetime,
                            &tag_nor_inputs, &decoder_tag_power);

                        cached_tag_calculations[tag_iteration_counter]
                            .decoder.delay = decoder_tag;
                        copy_powerDef(
                            &(cached_tag_calculations[tag_iteration_counter]
                                  .decoder.power),
                            decoder_tag_power);
                      ...
                      if (tag_iteration_counter < MAX_CACHE_ENTRIES &&
                        ...
                        copy_powerDef(&wordline_tag_power,
                                      current_cache_entry.wordline.power);
                      } else {
                        wordline_tag = wordline_tag_delay(
                            parameters->cache_size, parameters->block_size,
                            parameters->tag_associativity, Ntspd, Ntwl, Ntbl,
                            *NSubbanks, inrisetime, &outrisetime,
                            &wordline_tag_power);

                        cached_tag_calculations[tag_iteration_counter]
                            .wordline.delay = wordline_tag;
                        copy_powerDef(
                            &(cached_tag_calculations[tag_iteration_counter]
                                  .wordline.power),
                            wordline_tag_power);
                      ...
                      if (tag_iteration_counter < MAX_CACHE_ENTRIES &&
                        ...
                        copy_powerDef(&bitline_tag_power,
                                      current_cache_entry.bitline.power);
                      } else {
                        bitline_tag = bitline_tag_delay(
                      ...
                      if (valid_tag_cache_entries[tag_iteration_counter] ==
                          TRUE) {
                        current_cache_entry =
                            cached_tag_calculations[tag_iteration_counter];
                        sense_amp_tag = current_cache_entry.senseamp.delay;
                        copy_powerDef(&sense_amp_tag_power,
                                      current_cache_entry.senseamp.power);

                        outrisetime =
                            current_cache_entry.senseamp_outputrisetime;
                      } else {
                        sense_amp_tag = sense_amp_tag_delay(
                            parameters->cache_size, parameters->block_size,
                            parameters->tag_associativity, Ntwl, Ntbl, Ntspd,
                            *NSubbanks, inrisetime, &outrisetime,
                            &sense_amp_tag_power);
                        ...
                        cached_tag_calculations[tag_iteration_counter]
                            .senseamp_outputrisetime = outrisetime;

                        if (*NSubbanks == 1 &&
                            tag_iteration_counter < MAX_CACHE_ENTRIES) {
                      ...
                      compare_tag = half_compare_delay(
                          parameters->cache_size, parameters->block_size,
                          parameters->tag_associativity, Ntwl, Ntbl, Ntspd,
                          *NSubbanks, inrisetime, &outrisetime,
                          &compare_tag_power);
                      ...
                      if (parameters->tag_associativity == 1) {
                        mux_driver = 0;
                        valid_driver = valid_driver_delay(
                            parameters->cache_size, parameters->block_size,
                            parameters->tag_associativity,
                            parameters->fully_assoc, Ndbl, Ndwl, Nspd, Ntbl,
                            Ntwl, Ntspd, NSubbanks, inrisetime,
                            &valid_driver_power);

                        max_delay = MAX(max_delay, valid_driver);
                        time_till_compare = subbank_address_routing_delay +
                                            decoder_tag + wordline_tag +
                                            bitline_tag + sense_amp_tag;

                        time_till_select =
                            time_till_compare + compare_tag + valid_driver;

                        /*
                         * From the above info, calculate the total access time
                         */
                        /*If we're doing a pure SRAM table, then we don't need
                         * to take into account tags */
                        if (pure_sram_flag) {
                          access_time = before_mux + after_mux;
                        } else {
                          if (sequential_access_flag) {
                            /*dt: testing sequential access mode*/
                            /*approximating the time it takes the match and
                             * valid signal to travel to the edge of the tag
                             * array with decode_tag */
                            access_time = before_mux + time_till_compare +
                                          compare_tag + decoder_tag;
                          } else {
                            access_time =
                                MAX(before_mux + after_mux, time_till_select);
                          }
                        }
                        ...
                        if (fast_cache_access_flag || sequential_access_flag) {
                          mux_driver = 0;
                        } //||sequential_access_flag added by Shyam to fix the
                          //bug
                        // of mux driver delay being larger than access time in
                        // serial access mode now mux driver delay code won't be
                        // touched in serial access mode
                        else {
                          /* dualin mux driver - added for split comparator
                             - inverter replaced by nand gate */
                          mux_driver = mux_driver_delay_dualin(
                              parameters->cache_size, parameters->block_size,
                              parameters->tag_associativity, Ntbl, Ntspd,
                              inrisetime, &outrisetime, wirelength_v,
                              wirelength_h, &mux_driver_power);
                        }
                        ...
                        if (fast_cache_access_flag) {
                          /*dt: don't wait for the mux signal to travel to the
                            subarray, have the n ways routed to the edge of the
                            data array*/
                          time_till_select =
                              time_till_compare + compare_tag + selb;
                        } else {
                          time_till_select = time_till_compare + compare_tag +
                                             mux_driver + selb;
                        }

                        if (sequential_access_flag) {
                          /*dt: testing sequential access*/
                          /*approximating the time it takes the match and valid
                           * signal to travel to the edge of the tag array with
                           * decode_tag */
                          access_time = before_mux + time_till_compare +
                                        compare_tag + decoder_tag;
                        } else {
                          access_time =
                              MAX(before_mux, time_till_select) + after_mux;
                        }
                      }
                      ...
                      /*dt: replacing this with a simple calculation:
                                ...
                                senseamp
                                comparator
                                so those are the things which are going to limit
                         cycle time
                      */
                      cycle_time =
                          MAX(MAX(wordline_data + bitline_data + sense_amp_data,
                       ...
                       * the results by FUDGEFACTOR.  Normally, FUDGEFACTOR
                       * is 1.
                       */

                      // v4.1: All delay and energy components are no longer
                      // scaled by FUDGEFACTOR as they have already been
                      // computed for the input tech node.
                      allports = parameters->num_readwrite_ports +
                                 parameters->num_read_ports +
                                 parameters->num_write_ports;
                      allreadports = parameters->num_readwrite_ports +
                                     parameters->num_read_ports;
                      allwriteports = parameters->num_readwrite_ports +
                                      parameters->num_write_ports;
                      ...
                      mac_powerDef(&total_power, &subbank_address_routing_power,
                                   allports);
                      mac_powerDef(&total_power, &decoder_data_power, allports);
                      mac_powerDef(&total_power, &wordline_data_power,
                                   allports);
                      /*dt: We can have different numbers of read/write ports,
                      so the power numbers have to keep that in mind. Ports
                      which can do both reads and writes are counted as writes
                      for max power, because writes use full swing and thus use
                      more power. (Assuming full bitline swing is greater than
                      senseamp power) */
                      total_power.readOp.dynamic +=
                          bitline_data_power.readOp.dynamic * allreadports;
                      total_power.writeOp.dynamic +=
                          bitline_data_power.writeOp.dynamic * allwriteports;
                      /*dt: for multiported SRAM cells we assume NAND stacks on
                      all the pure read ports. We assume neglegible leakage for
                      these ports. The following code adjusts the leakage
                      numbers accordingly.*/
                      total_power.readOp.leakage +=
                          bitline_data_power.readOp.leakage *
                          allreadports; // changed to allreadports by Shyam.
                                        // Earlier this was allwriteports.
                      total_power.writeOp.leakage +=
                          bitline_data_power.writeOp.leakage * allwriteports;

                      mac_powerDef(&total_power, &sense_amp_data_power,
                                   allreadports);
                      mac_powerDef(&total_power, &total_out_driver_power,
                                   allreadports);
                      /*dt: Tags are only written to when a cache line is
                      evicted/replaced. The only bit which is written is the
                      dirty bit. The dirty bits are kept in the data array. */
                      // if on pure_sram_flag added by Shyam since tag power
                      // should not be added to total power in pure SRAM mode.
                      if (!pure_sram_flag) {
                        mac_powerDef(&total_power, &decoder_tag_power,
                                     allreadports);
                        mac_powerDef(&total_power, &wordline_tag_power,
                                     allreadports);
                        mac_powerDef(&total_power, &bitline_tag_power,
                                     allreadports);
                        mac_powerDef(&total_power, &sense_amp_tag_power,
                                     allreadports);
                        mac_powerDef(&total_power, &compare_tag_power,
                                     allreadports);
                        mac_powerDef(&total_power, &valid_driver_power,
                                     allreadports);
                        mac_powerDef(&total_power, &mux_driver_power,
                                     allreadports);
                        mac_powerDef(&total_power, &selb_power, allreadports);
                      }
                      mac_powerDef(&total_power, &data_output_power,
                                   allreadports);

                      reset_powerDef(&total_power_without_routing);
                      mac_powerDef(&total_power_without_routing, &total_power,
                                   1); // copy over and ..

                      // total_power.readOp.dynamic /= FUDGEFACTOR;
                      // total_power.writeOp.dynamic /= FUDGEFACTOR;
                      /*dt: Leakage isn't scaled with FUDGEFACTOR, because
                       * that's already done by the leakage model much more
                       * realistically */

                      mac_powerDef(&total_power_without_routing,
                                   &subbank_address_routing_power,
                                   -allports); // ... then subtract ..
                      mac_powerDef(&total_power_without_routing,
                                   &total_out_driver_power, -allreadports);
                      mac_powerDef(&total_power_without_routing,
                                   &valid_driver_power, -allreadports);

                      /*total_power_without_routing.readOp.dynamic *=
                      (*NSubbanks)/ FUDGEFACTOR;
                      total_power_without_routing.readOp.leakage *=
                      (*NSubbanks); total_power_without_routing.writeOp.dynamic
                      *= (*NSubbanks)/ FUDGEFACTOR;
                      total_power_without_routing.writeOp.leakage *=
                      (*NSubbanks);*/
                      ...
                      total_power_without_routing.readOp.leakage *=
                          (*NSubbanks);
                      total_power_without_routing.writeOp.dynamic *=
                          (*NSubbanks);
                      total_power_without_routing.writeOp.leakage *=
                          (*NSubbanks);
                      ...
                      // total_power_allbanks.readOp.dynamic =
                      // total_power_without_routing.readOp.dynamic +
                      // total_address_routing_power.readOp.dynamic /
                      // FUDGEFACTOR;
                      total_power_allbanks.readOp.dynamic =
                          total_power_without_routing.readOp.dynamic +
                          total_address_routing_power.readOp.dynamic;
                      total_power_allbanks.readOp.leakage =
                          total_power_without_routing.readOp.leakage +
                          total_address_routing_power.readOp.leakage;
                      // total_power_allbanks.writeOp.dynamic =
                      // total_power_without_routing.writeOp.dynamic +
                      // total_address_routing_power.writeOp.dynamic /
                      // FUDGEFACTOR;
                      total_power_allbanks.writeOp.dynamic =
                          total_power_without_routing.writeOp.dynamic +
                          total_address_routing_power.writeOp.dynamic;
                      total_power_allbanks.writeOp.leakage =
                          total_power_without_routing.writeOp.leakage +
                          total_address_routing_power.writeOp.leakage;

                      //      if (counter==1)
                      //        fprintf(stderr, "Pow - %f, Acc - %f, Pow - %f,
                      //        Acc - %f, Combo - %f\n", total_power*1e9,
                      //        access_time*1e9, total_power/result->max_power,
                      //        access_time/result->max_access_time,
                      //        total_power/result->max_power*access_time/result->max_access_time);
                      ...
                      if (counter == 1) {
                        if (objective_function(
                                ...
                                ((result->total_power.readOp.dynamic /
                                  result->cycle_time) +
                                 result->total_power.readOp.leakage +
                                 (result->total_power.writeOp.dynamic /
                                  result->cycle_time) +
                                 result->total_power.writeOp.leakage) /
                                    2.0 / result->max_power) >
                            objective_function(
                                1, 1, 1,
                                access_time / (result->max_access_time),
                                1.0 / efficiency,
                                ((total_power.readOp.dynamic / (cycle_time)) +
                                 total_power.readOp.leakage +
                                 (total_power.writeOp.dynamic / (cycle_time)) +
                                 total_power.writeOp.leakage) /
                                    2.0 /
                                    result->max_power)) // read and write
                                                        // dynamic energy
                                                        // components changed to
                                                        // power components by
                                                        // Shyam. Earlier
                        // read and write dynamic energies were being added to
                        // leakage power components and getting swamped
                        {
                          result->senseext_scale = senseext_scale;
                          copy_powerDef(&result->total_power, total_power);
                          copy_powerDef(&result->total_power_without_routing,
                                        total_power_without_routing);
                          // copy_and_div_powerDef(&result->total_routing_power,total_address_routing_power,FUDGEFACTOR);
                          copy_powerDef(&result->total_routing_power,
                                        total_address_routing_power);
                          copy_powerDef(&result->total_power_allbanks,
                                        total_power_allbanks);

                          result->subbank_address_routing_delay =
                              subbank_address_routing_delay / FUDGEFACTOR;
                          // copy_and_div_powerDef(&result->subbank_address_routing_power,subbank_address_routing_power,FUDGEFACTOR);
                          copy_powerDef(&result->subbank_address_routing_power,
                                        subbank_address_routing_power);
                          ...
                          FUDGEFACTOR; result->dec_tag_3to8 = decoder_tag_3to8 /
                          FUDGEFACTOR; result->dec_tag_inv = decoder_tag_inv /
                          FUDGEFACTOR; result->dec_data_driver =
                          decoder_data_driver / FUDGEFACTOR;
                          result->dec_data_3to8 = decoder_data_3to8 /
                          FUDGEFACTOR; result->dec_data_inv = decoder_data_inv /
                          FUDGEFACTOR; result->wordline_delay_data =
                          wordline_data / FUDGEFACTOR;
                          ...
                          result->decoder_delay_data = decoder_data;
                          copy_powerDef(&result->decoder_power_data,
                                        decoder_data_power);
                          result->decoder_delay_tag = decoder_tag;
                          copy_powerDef(&result->decoder_power_tag,
                                        decoder_tag_power);
                          result->dec_tag_driver = decoder_tag_driver;
                          ...
                          copy_powerDef(&result->wordline_power_data,
                                        wordline_data_power);
                          result->wordline_delay_tag = wordline_tag;
                          copy_powerDef(&result->wordline_power_tag,
                                        wordline_tag_power);
                          result->bitline_delay_data = bitline_data;
                          copy_powerDef(&result->bitline_power_data,
                                        bitline_data_power);
                          result->bitline_delay_tag = bitline_tag;
                          copy_powerDef(&result->bitline_power_tag,
                                        bitline_tag_power);
                          result->sense_amp_delay_data = sense_amp_data;
                          copy_powerDef(&result->sense_amp_power_data,
                                        sense_amp_data_power);
                          result->sense_amp_delay_tag = sense_amp_tag;
                          copy_powerDef(&result->sense_amp_power_tag,
                                        sense_amp_tag_power);
                          result->total_out_driver_delay_data =
                              total_out_driver;
                          copy_powerDef(&result->total_out_driver_power_data,
                                        total_out_driver_power);
                          result->compare_part_delay = compare_tag;
                          copy_powerDef(&result->compare_part_power,
                                        compare_tag_power);
                          result->drive_mux_delay = mux_driver;
                          copy_powerDef(&result->drive_mux_power,
                                        mux_driver_power);
                          result->selb_delay = selb;
                          copy_powerDef(&result->selb_power, selb_power);
                          result->drive_valid_delay = valid_driver;
                          copy_powerDef(&result->drive_valid_power,
                                        valid_driver_power);
                          result->data_output_delay = data_output;
                          copy_powerDef(&result->data_output_power,
                                        data_output_power);
                          result->precharge_delay = precharge_del;
                          ...
                          // v4.1: No longer using calculate_area function as
                          // area has already been computed for the given tech
                          // node
                          ...
                          arearesult->efficiency =
                              (*NSubbanks) *
                              (area_all_dataramcells + area_all_tagramcells) *
                              100 /
                              (arearesult_subbanked->height *
                               arearesult_subbanked->width / 100000000.0);
                          ...
                          arearesult->aspect_ratio_total =
                              (arearesult->aspect_ratio_total > 1.0)
                                  ? (arearesult->aspect_ratio_total)
                                  : 1.0 / (arearesult->aspect_ratio_total);
                        ...
                        if (result->max_power <
                          ...
                          result->max_power =
                              ((total_power.readOp.dynamic / (cycle_time)) +
                               ...
                               total_power.writeOp.leakage) /
                              2.0;
                          // read and write dynamic energy components changed to
                          // power components by Shyam. Earlier read and write
                          // dynamic energies were being added to leakage power
                          // components and getting swamped
                        }
      ...
      for (Ndbl = 1; Ndbl <= MAXDATAN; Ndbl = Ndbl * 2) {
        ...
        if (data_organizational_parameters_valid(
                parameters->block_size, 1, parameters->cache_size, Ndwl, Ndbl,
                Nspd, parameters->fully_assoc, (*NSubbanks))) {
          ...
          area_subbanked(ADDRESS_BITS, BITOUT, parameters->num_readwrite_ports,
                         parameters->num_read_ports,
                         parameters->num_write_ports, Ndbl, Ndwl, Nspd, Ntbl,
                         Ntwl, Ntspd, *NSubbanks, parameters,
                         &arearesult_subbanked_temp, &arearesult_temp);

          Subbank_Efficiency = (area_all_dataramcells + area_all_tagramcells) *
                               100 / (arearesult_temp.totalarea / 100000000.0);

          // v4.1: No longer using calculate_area function as area has already
          // been computed for the given tech node
          /*Total_Efficiency =
          ...
          arearesult_temp.efficiency = efficiency;
          aspect_ratio_total_temp = (arearesult_subbanked_temp.height /
                                     arearesult_subbanked_temp.width);
          aspect_ratio_total_temp = (aspect_ratio_total_temp > 1.0)
                                        ? (aspect_ratio_total_temp)
                                        : 1.0 / (aspect_ratio_total_temp);
          ...
          subbank_dim(parameters->cache_size, parameters->block_size,
                      parameters->data_associativity, parameters->fully_assoc,
                      Ndbl, Ndwl, Nspd, Ntbl, Ntwl, Ntspd, *NSubbanks, &bank_h,
                      &bank_v);

          reset_powerDef(&subbank_address_routing_power);
          /*dt: this has to be reset on every loop iteration, or else we
            implicitly reuse the risetime from the end of the last loop
            iteration!*/
          inrisetime = addr_inrisetime = 0;

          subbanks_routing_power(
              parameters->fully_assoc, parameters->data_associativity,
              *NSubbanks, &bank_h, &bank_v, &total_address_routing_power);

          if (*NSubbanks > 2) {
            subbank_address_routing_delay = address_routing_delay(
                parameters->cache_size, parameters->block_size,
                parameters->data_associativity, parameters->fully_assoc, Ndwl,
                Ndbl, Nspd, Ntwl, Ntbl, Ntspd, NSubbanks, &outrisetime,
                &subbank_address_routing_power);
          }
          ...
          reset_powerDef(&decoder_data_power);
          decoder_data = fa_tag_delay(
              parameters->cache_size, parameters->block_size, Ntwl, Ntbl, Ntspd,
              &tag_delay_part1, &tag_delay_part2, &tag_delay_part3,
              &tag_delay_part4, &tag_delay_part5, &tag_delay_part6,
          ...
          Clinefa = (gatecappass(Wmemcella, (BitWidth - 2 * Wmemcella) / 2.0) +
                     gatecappass(Wmemcella, (BitWidth - 2 * Wmemcella) / 2.0) +
                     Cwordmetal) *
                    colsfa;
          ...
          wordline_data = wordline_delay(
              parameters->cache_size, parameters->block_size, 1, Ndwl, Ndbl,
              Nspd, inrisetime, &outrisetime, &wordline_data_power);

          inrisetime = outrisetime;
          max_delay = MAX(max_delay, wordline_data);
          /*dt: assuming that the precharge delay is equal to decode + wordline
           * delay */
          Tpre += wordline_data;

          reset_powerDef(&bitline_data_power);
          bitline_data = bitline_delay(
              parameters->cache_size, 1, parameters->block_size, Ndwl, Ndbl,
              Nspd, inrisetime, &outrisetime, &bitline_data_power, Tpre);
...

          {
            ...
            sense_amp_data = sense_amp_delay(
                parameters->cache_size, parameters->block_size,
                parameters->data_associativity, Ndwl, Ndbl, Nspd, inrisetime,
                &outrisetime, &sense_amp_data_power);
          ...
          reset_powerDef(&data_output_power);
          data_output =
              dataoutput_delay(parameters->cache_size, parameters->block_size,
                               1, parameters->fully_assoc, Ndbl, Nspd, Ndwl,
                               inrisetime, &outrisetime, &data_output_power);
          ...
          subbank_routing_length(parameters->cache_size, parameters->block_size,
                                 parameters->data_associativity,
                                 parameters->fully_assoc, Ndbl, Nspd, Ndwl,
                                 Ntbl, Ntwl, Ntspd, *NSubbanks, &subbank_v,
                                 &subbank_h);

          if (*NSubbanks > 2) {
            total_out_driver = senseext_driver_delay(
                parameters->data_associativity, parameters->fully_assoc,
                inrisetime, &outrisetime, subbank_v, subbank_h,
                &total_out_driver_power);
          }

          /*
                          total_out_driver =
             total_out_driver_delay(parameters->cache_size,
          ...
          */
          ...
          access_time = subbank_address_routing_delay + decoder_data +
                        wordline_data + bitline_data + sense_amp_data +
          ...
          cycle_time = MAX(MAX(wordline_data + bitline_data + sense_amp_data,
                               wordline_tag + bitline_tag + sense_amp_tag),
                           compare_tag);
          ...
          allwriteports =
              parameters->num_readwrite_ports + parameters->num_write_ports;
          ...
          /*dt: We can have different numbers of read/write ports, so the power
          numbers have to keep that in mind. Ports which can do both reads and
          writes are counted as writes for max power, because writes use full
          swing and thus use more power. (Assuming full bitline swing is greater
          than senseamp power) */
          total_power.readOp.dynamic +=
              bitline_data_power.readOp.dynamic * allreadports;
          total_power.writeOp.dynamic +=
              bitline_data_power.writeOp.dynamic * allwriteports;
          /*dt: for multiported SRAM cells we assume NAND stacks on all the pure
          read ports. We assume neglegible leakage for these ports. The
          following code adjusts the leakage numbers accordingly.*/
          total_power.readOp.leakage +=
              bitline_data_power.readOp.leakage * allwriteports;
          total_power.writeOp.leakage +=
              bitline_data_power.writeOp.leakage * allwriteports;

          mac_powerDef(&total_power, &sense_amp_data_power, allreadports);
          mac_powerDef(&total_power, &total_out_driver_power, allreadports);
          /*dt: in a fully associative cache we don't have tag decoders,
           * wordlines or bitlines, etc. . Only CAM */
          reset_powerDef(&decoder_tag_power);
          reset_powerDef(&wordline_tag_power);
          reset_powerDef(&bitline_tag_power);
          reset_powerDef(&sense_amp_tag_power);
          ...
          // total_power.readOp.dynamic /= FUDGEFACTOR;
          // total_power.writeOp.dynamic /= FUDGEFACTOR;
          /*dt: Leakage isn't scaled with FUDGEFACTOR, because that's already
           * done by the leakage model much more realistically */

          mac_powerDef(&total_power_without_routing,
                       &subbank_address_routing_power,
                       -allports); // ... then subtract ..
          mac_powerDef(&total_power_without_routing, &total_out_driver_power,
                       -allreadports);
          mac_powerDef(&total_power_without_routing, &valid_driver_power,
                       -allreadports);
          ...
          // total_power_allbanks.readOp.dynamic =
          // total_power_without_routing.readOp.dynamic +
          // total_address_routing_power.readOp.dynamic / FUDGEFACTOR;
          total_power_allbanks.readOp.dynamic =
              total_power_without_routing.readOp.dynamic +
              total_address_routing_power.readOp.dynamic;
          total_power_allbanks.readOp.leakage =
          ...
          // total_power_allbanks.writeOp.dynamic =
          // total_power_without_routing.writeOp.dynamic +
          // total_address_routing_power.writeOp.dynamic / FUDGEFACTOR;
          total_power_allbanks.writeOp.dynamic =
              total_power_without_routing.writeOp.dynamic +
              total_address_routing_power.writeOp.dynamic;
          total_power_allbanks.writeOp.leakage =
          ...
          if (counter == 1) {
            // if
            // ((result->total_power/result->max_power)/2+(result->access_time/result->max_access_time)
            // >
            // ((total_power/result->max_power)/2+access_time/(result->max_access_time*FUDGEFACTOR)))
            // {

            if (objective_function(
                    1, 1, 1, result->access_time / result->max_access_time,
                    1.0 / arearesult->efficiency,
                    ((result->total_power.readOp.dynamic / result->cycle_time) +
                     result->total_power.readOp.leakage +
                     (result->total_power.writeOp.dynamic /
                      result->cycle_time) +
                     result->total_power.writeOp.leakage) /
                        2.0 / result->max_power) >
                objective_function(
                    1, 1, 1, access_time / (result->max_access_time),
                    1.0 / efficiency,
                    ((total_power.readOp.dynamic / (cycle_time)) +
                     total_power.readOp.leakage +
                     (total_power.writeOp.dynamic / (cycle_time)) +
                     total_power.writeOp.leakage) /
                        2.0 /
                        result->max_power)) // read and write dynamic energy
                                            // components changed to power
                                            // components by Shyam. Earlier read
                                            // and write dynamic energies were
                                            // being added to leakage power
                                            // components and getting swamped

            {
              // if
              // ((result->total_power/result->max_power)/2+(result->access_time/result->max_access_time)
              // + (min_efficiency/arearesult->efficiency)/4 +
              // (arearesult->aspect_ratio_total/max_aspect_ratio_total)/3 >
              // ((total_power/result->max_power)/2+access_time/(result->max_access_time*FUDGEFACTOR)+
              // (min_efficiency/efficiency)/4 +
              // (arearesult_temp.aspect_ratio_total/max_aspect_ratio_total)/3))
              // {
              ...
              area_subbanked(
                  ADDRESS_BITS, BITOUT, parameters->num_readwrite_ports,
                  parameters->num_read_ports, parameters->num_write_ports, Ndbl,
                  Ndwl, Nspd, Ntbl, Ntwl, Ntspd, *NSubbanks, parameters,
                  arearesult_subbanked, arearesult);

              // v4.1: No longer using calculate_area function as area has
              // already been computed for the given tech node
              /*arearesult->efficiency =
                (*NSubbanks) * (area_all_dataramcells +
                                area_all_tagramcells) * 100 /
            ...
            if (result->max_power <
              ...
              result->max_power =
                  ((total_power.readOp.dynamic / (cycle_time)) +
                   ...
                   total_power.writeOp.leakage) /
                  2.0;
            } // read and write dynamic energy components changed to power
              // components by Shyam. Earlier read and write dynamic energies
              // were being added to leakage power components and getting
              // swamped
            if (arearesult_temp.max_efficiency < efficiency) {
              arearesult_temp.max_efficiency = efficiency;
              max_efficiency = efficiency;
