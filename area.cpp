...
// v4.1: Earlier all the dimensions (length/width/thickness) of the transistors
// and wires were calculated for the 0.8 micron process and then scaled to the
// input techology. Now all dimensions are calculated directly for the input
// technology, so area no longer needs to be scaled to the input technology.
...
area_type inverter_area(double Widthp, double Widthn) {
  ...
  Width_n = (foldn) ? (3 * Widthcontact + 2 * (Wpoly + 2 * ptocontact))
                    : (2 * Widthcontact + Wpoly + 2 * ptocontact);
  Width_p = (foldp) ? (3 * Widthcontact + 2 * (Wpoly + 2 * ptocontact))
                    : (2 * Widthcontact + Wpoly + 2 * ptocontact);
...
area_type
subarraymem_area(int C, int B, int A, int Ndbl, int Ndwl, double Nspd, int RWP,
                 int ERP, int EWP, int NSER,
                 double techscaling_factor) /* returns area of subarray */
{
  area_type memarea;
  int noof_rows, noof_colns;

  // v4.1: Fixing double->int type conversion problems. EPSILON is added below
  // to make sure the final int value is the correct one noof_rows = (C / (B * A
  // * Ndbl * Nspd)); noof_colns = (8 * B * A * Nspd / Ndwl);
  noof_rows = (int)((C / (B * A * Ndbl * Nspd)) + EPSILON);
  noof_colns = (int)((8 * B * A * Nspd / Ndwl) + EPSILON);

  memarea.height =
      ceil((double)(noof_rows) / 16.0) * stitch_ramv +
      (BitHeight1x1 + Widthtrack * 2 * (RWP + ERP + EWP - 1)) * noof_rows;
  memarea.width =
      noof_colns *
      (BitWidth1x1 +
       (Widthtrack * 2 * (RWP + (ERP - NSER) + EWP - 1) + Widthtrack * NSER));
  /* dt
  was : memarea.height =
  ceil((double)(noof_rows)/16.0)*stitch_ramv+(BitHeight16x2+2*Widthtrack*2*(RWP+ERP+EWP-1))*ceil((double)(noof_rows)/2.0);
            memarea.width  =
  ceil((double)(noof_colns)/16.0)*(BitWidth16x2+16*(Widthtrack*2*(RWP+(ERP-NSER)+EWP-1)+Widthtrack*NSER));
  now : use single cell for width and height
  */

  // area_all_dataramcells =
  // Ndwl * Ndbl * calculate_area (memarea, techscaling_factor) *
  // CONVERT_TO_MMSQUARE;
  area_all_dataramcells =
      Ndwl * Ndbl * memarea.height * memarea.width * CONVERT_TO_MMSQUARE;
  return (memarea);
}

area_type decodemem_row(int C, int B, int A, int Ndbl, double Nspd, int Ndwl,
                        int RWP, int ERP,
                        int EWP) /* returns area of post decode */
{
  ...
  If we have associativity A, then all the ways are mapped to one wordline ->
  8*B*A If we mapped more than one set to each wordline (if Nspd > 1) than the
  wordline is longer again -> 8*B*A*Nspd If we have subdivided the global
  wordline into segments (if Ndwl > 1) than the local wordline is shorter ->
  8*B*A*Nspd/Ndwl
  */
  // v4.1: Fixing double->int type conversion problems. EPSILON is added below
  // to make sure the final int value is the correct one noof_colns = 8 * B * A
  // * Nspd / Ndwl;
  noof_colns = (int)(8 * B * A * Nspd / Ndwl + EPSILON);
  ...
  numstack = (int)ceil(
      (1.0 / 3.0) *
      logtwo_area((double)((double)C / (double)(B * A * Ndbl * Nspd))));
  if (numstack == 0)
    numstack = 1;
  if (numstack > 5)
    numstack = 5;
  ...
  /*
        was: postdecodearea.height =
     (BitHeight16x2+2*Widthtrack*2*(RWP+ERP+EWP-1));
  */
  postdecodearea.height =
      (2 * BitHeight1x1 + 2 * Widthtrack * 2 * (RWP + ERP + EWP - 1));
  postdecodearea.width =
      (decodeNORwidth + decinv.height + worddriveinv.height) *
      (RWP + ERP + EWP);
  return (postdecodearea);
}

// v4.1: Making noof_rows double since the variable colns_datasubarray is
// function area was made double and is used as an argument corresponding to
// noof_rows in function calls to predecode_area area_type predecode_area (int
// noof_rows,int RWP,int ERP,int EWP)	/*returns the area of predecode */
...

{
  ...
  int N3to8;
  // v4.1: noof_rows can be less than 1 now since because Nspd can be a
  // fraction. When noof_rows is less than 1 making N3to8 1. This is not clean
...
// v4.1: Making noof_rows double since the variable colns_datasubarray is
// function area was made double and is used as an argument corresponding to
// noof_rows in function calls to postdecode_area
...
area_type colmux(int Ndbl, double Nspd, int RWP, int ERP, int EWP,
                 int NSER) /* gives the height of the colmux */
{
  area_type colmux_area;
  colmux_area.height =
      (2 * Wiso + 3 * (2 * Widthcontact + 1 / FUDGEFACTOR)) *
      (RWP + ERP + EWP); // Shyam: Need to understand what the +1 is for
  colmux_area.width =
...
area_type precharge(int Ndbl, double Nspd, int RWP, int ERP, int EWP,
                    int NSER) {
  area_type precharge_area;
  if (Ndbl * Nspd > 1) {
    precharge_area.height =
        (Wbitpreequ + 2 * Wbitdropv + Wwrite +
         2 * (2 * Widthcontact + 1 / FUDGEFACTOR) + 3 * Widthptondiff) *
        0.5 * (RWP + EWP);
    ...
    precharge_area.height =
        (Wbitpreequ + 2 * Wbitdropv + Wwrite +
         2 * (2 * Widthcontact + 1 / FUDGEFACTOR) + 3 * Widthptondiff) *
        (RWP + EWP);
    precharge_area.width = BitWidth +
                           Widthtrack * 2 * (RWP + (ERP - NSER) + EWP - 1) +
                           Widthtrack * NSER;
  }
...
area_type
subarraytag_area(int baddr, int C, int B, int A, int Ntdbl, int Ntdwl,
                 int Ntspd, double NSubbanks, int RWP, int ERP, int EWP,
                 int NSER,
                 double techscaling_factor) /* returns area of subarray */
{
  area_type tagarea;
  ...
  tagarea.width =
      noof_colns *
      (BitWidth1x1 +
       (Widthtrack * 2 * (RWP + (ERP - conservative_NSER) + EWP - 1) +
        Widthtrack * conservative_NSER));
  /*
  was:
  tagarea.height =
  ceil((double)(noof_rows)/16.0)*stitch_ramv+(BitHeight16x2+2*Widthtrack*2*(RWP+ERP+EWP-1))*ceil((double)(noof_rows)/2.0);
  tagarea.width =
  ceil((double)(noof_colns)/16.0)*(BitWidth16x2+16*(Widthtrack*2*(RWP+(ERP-conservative_NSER)+EWP-1)+Widthtrack*conservative_NSER));
  now:
  ...
  */
  ...
  area_all_tagramcells =
      Ntdwl * Ntdbl * tagarea.height * tagarea.width * CONVERT_TO_MMSQUARE;
  return (tagarea);
}

area_type decodetag_row(int baddr, int C, int B, int A, int Ntdbl, int Ntspd,
                        int Ntdwl, double NSubbanks, int RWP, int ERP,
                        int EWP) /* returns area of post decode */
{
  ...
  // v4.1: Fixing double->int type conversion problems. EPSILON is added below
  // to make sure the final int value is the correct one Tagbits = baddr -
  // logtwo_area ((double) (C)) + logtwo_area ((double) (A)) + 2 - logtwo_area
  // ((double) (NSubbanks));
  Tagbits = (int)(baddr - logtwo_area((double)(C)) + logtwo_area((double)(A)) +
                  2 - logtwo_area((double)(NSubbanks)) + EPSILON);
  ...
  numstack = (int)(ceil(
      (1.0 / 3.0) *
      logtwo_area((double)((double)C / (double)(B * A * Ntdbl * Ntspd)))));
  if (numstack == 0)
    numstack = 1;
  if (numstack > 5)
    numstack = 5;
...
area_type comparatorbit(int RWP, int ERP, int EWP) {
  ...
  compbit_area.height =
      (Wcompn + 2 * (2 * Widthcontact + 1 / FUDGEFACTOR)) * (RWP + ERP);
  return (compbit_area);
}
...
area_type muxdriverdecode(int B, int b0, int RWP, int ERP, int EWP) {
  ...
  muxdrvdecode_area.width = predecode.height + postdecode.width +
                            noof_rows * Widthtrack * (RWP + ERP + EWP);
  muxdrvdecode_area.height = MAX(predecode.width, postdecode.height);
  return (muxdrvdecode_area);
}

area_type muxdrvsig(int A, int B, int b0) /* generates the 8B/b0*A signals */
{
  int noof_rows;
  ...
  outdrvsig_area.height = 0.5 * (WmuxdrvNORn + WmuxdrvNORp) + 9 * Widthcontact +
                          0.5 * (Wmuxdrv3n + Wmuxdrv3p) + Widthptondiff +
                          3 * Widthcontact;
  ...
  case 2:
    muxdrvsig_area.height =
        outdrvsig_area.height * 2 + noof_rows * Widthtrack * 3 + A * Widthtrack;
    muxdrvsig_area.width =
        outdrvsig_area.width + noof_rows * Widthtrack + A * Widthtrack;
    break;
  case 4:
    muxdrvsig_area.height =
        outdrvsig_area.height * 2 + noof_rows * Widthtrack * 5 + A * Widthtrack;
    muxdrvsig_area.width =
        outdrvsig_area.width * 2 + noof_rows * Widthtrack + A * Widthtrack;
    break;
  case 8:
    muxdrvsig_area.height =
        outdrvsig_area.height * 2 + noof_rows * Widthtrack * 9 + A * Widthtrack;
    muxdrvsig_area.width =
        outdrvsig_area.width * 4 + noof_rows * Widthtrack + A * Widthtrack;
    break;
  case 16:
    muxdrvsig_area.height = outdrvsig_area.height * 4 +
                            noof_rows * Widthtrack * 18 + A * Widthtrack;
    muxdrvsig_area.width =
        outdrvsig_area.width * 4 + noof_rows * Widthtrack + A * Widthtrack;
    break;
  case 32:
    muxdrvsig_area.height = outdrvsig_area.height * 4 +
                            noof_rows * Widthtrack * 35 + 2 * A * Widthtrack;
    muxdrvsig_area.width = 2 * (outdrvsig_area.width * 4 +
                                noof_rows * Widthtrack + A * Widthtrack);
    break;
  default:
...
area_type datasubarray(int C, int B, int A, int Ndbl, int Ndwl, double Nspd,
  ...
  mem_area = subarraymem_area(C, B, A, Ndbl, Ndwl, Nspd, RWP, ERP, EWP, NSER,
                              techscaling_factor);
  ...
  datasubarray_area.height = mem_area.height + colmux_area.height +
                             precharge_area.height + senseamp_area.height +
                             DatainvHeight * (RWP + EWP) +
...
area_type datasubblock(int C, int B, int A, int Ndbl, int Ndwl, double Nspd,
                       int SB, int b0, int RWP, int ERP, int EWP, int NSER,
                       double techscaling_factor) {
  int N3to8;
  ...
  colmuxtracks_rem = (Ndbl * Nspd > tracks_precharge_p)
                         ? (Ndbl * Nspd - tracks_precharge_p)
                         : 0;
  outrdrvtracks_rem = ((2 * B * A) / (b0) > tracks_outdrvselinv_p)
                          ? ((2 * B * A) / (b0)-tracks_outdrvselinv_p)
                          : 0;
  writeseltracks_rem = ((2 * B * A) / (b0) > tracks_precharge_nx2)
                           ? ((2 * B * A) / (b0)-tracks_precharge_nx2)
                           : 0;
  // v4.1: using integer casting below
  // N3to8 =
  // ceil ((1.0 / 3.0) * logtwo_area ((double) (C / (B * A * Ndbl * Nspd))));
  N3to8 = (int)(ceil((1.0 / 3.0) *
                     logtwo_area((double)(C / (B * A * Ndbl * Nspd)))));
  if (N3to8 == 0) {
    N3to8 = 1;
  }
  ...
  tracks_h =
      Widthtrack *
      (N3to8 * 8 * (RWP + ERP + EWP) + (RWP + EWP) * colmuxtracks_rem +
       Ndbl * Nspd * ERP + 4 * outrdrvtracks_rem * (RWP + ERP) +
       4 * writeseltracks_rem * (RWP + EWP) + (RWP + ERP + EWP) * b0 / SB_);
  tracks_w = Widthtrack * (N3to8 * 8) * (RWP + ERP + EWP);
  datasubarray_area = datasubarray(C, B, A, Ndbl, Ndwl, Nspd, RWP, ERP, EWP,
                                   NSER, techscaling_factor);
...
area_type dataarray(int C, int B, int A, int Ndbl, int Ndwl, double Nspd,
  ...
  datasubarray_area = datasubarray(C, B, A, Ndbl, Ndwl, Nspd, RWP, ERP, EWP,
                                   NSER, techscaling_factor);
  datasubblock_area = datasubblock(C, B, A, Ndbl, Ndwl, Nspd, SB, b0, RWP, ERP,
                                   EWP, NSER, techscaling_factor);
  // area_all_datasubarrays =
  // Ndbl * Ndwl * calculate_area (datasubarray_area,
  // techscaling_factor) * CONVERT_TO_MMSQUARE;
  area_all_datasubarrays = Ndbl * Ndwl * datasubarray_area.height *
                           datasubarray_area.width * CONVERT_TO_MMSQUARE;
  ...
  if (SB == 0) {
    ...
    total_width = 2 * datasubblock_area.width + fixed_tracks_internal +
                  variable_tracks / 2;
  } else if (SB == 8) {
    total_height = 2 * datasubblock_area.height + fixed_tracks_internal +
                   variable_tracks / 2;
    total_width = 2 * (2 * datasubblock_area.width + variable_tracks / 4) +
    ...
    if (htree % 2 == 0) {
      for (i = 0; i <= iter_height; i++) {
        if (i == iter_height) {
          total_height = 2 * inter_height + data / blocks * multiplier +
                         fixed_tracks_external;
          total_width = 2 * inter_width + data / (2 * blocks) * multiplier +
  ...
  temp_aspect = ((temp.height / temp.width) > 1.0)
                    ? (temp.height / temp.width)
                    : 1.0 / (temp.height / temp.width);
  aspect_ratio_data =
      ((dataarray_area.height / dataarray_area.width) > 1.0)
          ? (dataarray_area.height / dataarray_area.width)
          : 1.0 / (dataarray_area.height / dataarray_area.width);
...
area_type tagsubarray(int baddr, int C, int B, int A, int Ndbl, int Ndwl,
                      double Nspd, double NSubbanks, int RWP, int ERP, int EWP,
  ...
  conservative_NSER = 0;
  tag_area = subarraytag_area(baddr, C, B, A, Ndbl, Ndwl, Nspd, NSubbanks, RWP,
                              ERP, EWP, conservative_NSER, techscaling_factor);
  postdecode_area =
      decodetag_row(baddr, C, B, A, Ndbl, Nspd, Ndwl, NSubbanks, RWP, ERP, EWP);
  ...
  tagsubarray_area.height = tag_area.height + colmux_area.height +
                            precharge_area.height + senseamp_area.height +
                            comp_area.height;
...
area_type tagsubblock(int baddr, int C, int B, int A, int Ndbl, int Ndwl,
                      double Nspd, double NSubbanks, int SB, int RWP, int ERP,
  ...
  // v4.1: Fixing double->int type conversion problems. EPSILON is added below
  // to make sure the final int value is the correct one T = baddr - logtwo_area
  // ((double) (C)) + logtwo_area ((double) (A)) + 2 - logtwo_area ((double)
  // (NSubbanks));
  T = (int)(baddr - logtwo_area((double)(C)) + logtwo_area((double)(A)) + 2 -
  ...
  colmuxtracks_rem = (Ndbl * Nspd > tracks_precharge_p)
                         ...
                         : 0;
  /*writeseltracks_rem = ((2*B*A)/(b0) > tracks_precharge_nx2) ?
   * ((2*B*A)/(b0)-tracks_precharge_nx2) : 0; */

  tracks_h = Widthtrack * (N3to8 * 8 * (RWP + ERP + EWP) +
                           (RWP + EWP) * colmuxtracks_rem + Ndbl * Nspd * ERP +
                           (RWP + ERP + EWP) * T / SB_ + (ERP + RWP) * A);
  tracks_w = Widthtrack * (N3to8 * 8) * (RWP + ERP + EWP);
  tagsubarray_area =
...
area_type tagarray(int baddr, int C, int B, int A, int Ndbl, int Ndwl,
                   double Nspd, double NSubbanks, int RWP, int ERP, int EWP,
                   int NSER, double techscaling_factor) {
  // int SB, CSB, N3to8, T;
  int SB, N3to8, T;
  ...
  if (!force_tag) {
    // T = ADDRESS_BITS +
    // EXTRA_TAG_BITS-(int)logtwo((double)C)+(int)logtwo((double)A)-(int)(logtwo(NSubbanks));
    T = (int)(ADDRESS_BITS + EXTRA_TAG_BITS - (int)logtwo((double)C) +
              (int)logtwo((double)A) - (int)(logtwo(NSubbanks)) + EPSILON);
  } else {
    T = force_tag_size;
  }
  ...
  // techscaling_factor) * CONVERT_TO_MMSQUARE;
  area_all_tagsubarrays = Ndbl * Ndwl * tagsubarray_area.height *
                          tagsubarray_area.width * CONVERT_TO_MMSQUARE;
  ...
  if (SB == 4) {
    ...
    total_width = 2 * tagsubblock_area.width + fixed_tracks_internal +
                  variable_tracks / 2;
  }
  if (SB == 8) {
    total_height = 2 * tagsubblock_area.height + fixed_tracks_internal +
                   variable_tracks / 2;
    total_width = 2 * (2 * tagsubblock_area.width + variable_tracks / 4) +
  ...
  if (SB > 8) {
    ...
    if (htree % 2 == 0) {
      for (i = 0; i <= iter_height; i++) {
        if (i == iter_height) {
          total_height = 2 * inter_height + tag / blocks * multiplier +
                         fixed_tracks_external;
          total_width = 2 * inter_width + tag / (2 * blocks) * multiplier +
                        fixed_tracks_internal;
        } else {
          total_height = 2 * inter_height + tag / blocks * multiplier +
                         fixed_tracks_internal;
  ...
  temp_aspect = ((temp.height / temp.width) > 1.0)
                    ? (temp.height / temp.width)
                    : 1.0 / (temp.height / temp.width);
  aspect_ratio_tag = ((tagarray_area.height / tagarray_area.width) > 1.0)
                         ? (tagarray_area.height / tagarray_area.width)
                         : 1.0 / (tagarray_area.height / tagarray_area.width);
...
void area(int baddr, int b0, int Ndbl, int Ndwl, double Nspd, int Ntbl,
  ...
  // v4.1. Making colns_datasubarray double datatype since it is dependent on
  // Nspd which is double. Note that based on its usage below colns_datasubarray
  // is a misnomer. colns_datasubarray is actually the degree of output muxing.
  double colns_datasubarray;
  int rows_tagsubarray, colns_tagsubarray;

  rows_datasubarray = (parameters->cache_size /
                       ((parameters->block_size) *
                        (parameters->data_associativity) * Ndbl * Nspd));
  colns_datasubarray = (Ndbl * Nspd);
  rows_tagsubarray = (parameters->cache_size /
                      ((parameters->block_size) *
                       (parameters->tag_associativity) * Ntbl * Ntspd));
  colns_tagsubarray = (Ntbl * Ntspd);

  result->dataarray_area = dataarray(
      ...
      parameters->num_write_ports, parameters->num_single_ended_read_ports,
  ...
  // if-else on pure_sram_flag added by Shyam so that tag array area not
  // calculated in pure SRAM mode
  if (!pure_sram_flag) {

    result->tagarray_area = tagarray(
        baddr, parameters->cache_size, parameters->block_size,
    ...
    result->tagcolmuxpredecode_area =
        predecode_area(colns_tagsubarray, parameters->num_readwrite_ports,
                       parameters->num_read_ports, parameters->num_write_ports);
    result->tagcolmuxpostdecode_area = postdecode_area(
        colns_tagsubarray, parameters->num_readwrite_ports,
        parameters->num_read_ports, parameters->num_write_ports);
    result->tagoutdrvdecode_area = muxdriverdecode(
        parameters->block_size, b0, parameters->num_readwrite_ports,
        parameters->num_read_ports, parameters->num_write_ports);
  ...
  result->totalarea =
      result->dataarray_area.height * result->dataarray_area.width +
      result->datapredecode_area.height * result->datapredecode_area.width +
      result->datacolmuxpredecode_area.height *
          result->datacolmuxpredecode_area.width +
      result->datacolmuxpostdecode_area.height *
          result->datacolmuxpostdecode_area.width +
      (parameters->num_readwrite_ports + parameters->num_write_ports) *
          result->datawritesig_area.height * result->datawritesig_area.width +
      result->tagarray_area.height * result->tagarray_area.width +
      result->tagpredecode_area.height * result->tagpredecode_area.width +
      result->tagcolmuxpredecode_area.height *
          result->tagcolmuxpredecode_area.width +
      result->tagcolmuxpostdecode_area.height *
          result->tagcolmuxpostdecode_area.width +
      result->tagoutdrvdecode_area.height * result->tagoutdrvdecode_area.width +
      (parameters->num_readwrite_ports + parameters->num_read_ports) *
          result->tagoutdrvsig_area.height * result->tagoutdrvsig_area.width;
}

area_type fadecode_row(int C, int B, int Ndbl, int RWP, int ERP,
                       int EWP) /*returns area of post decode */
{
  int numstack;
                     ...
                     double techscaling_factor) /* returns area of subarray */
{
  ...
  if ((RWP == 1) && (ERP == 0) && (EWP == 0)) {
    ...
    if ((RWP == 1) && (ERP == 1) && (EWP == 0)) {
      ...
      if ((RWP == 1) && (ERP == 0) && (EWP == 1)) {
        ...
        if (RWP + EWP >= 2) {
          widthoverhead = FAWidthIncrPer_first_rw_or_w_port +
                          (RWP + EWP - 2) * FAWidthIncrPer_later_rw_or_w_port +
                          ERP * FAWidthIncrPer_later_r_port;
          heightoverhead =
              FAHeightIncrPer_first_rw_or_w_port +
              (RWP + EWP - 2) * FAHeightIncrPer_later_rw_or_w_port +
              ERP * FAHeightIncrPer_later_r_port;
        } else {
          if ((RWP == 0) && (EWP == 0)) {
            widthoverhead = FAWidthIncrPer_first_r_port +
                            (ERP - 1) * FAWidthIncrPer_later_r_port;
            heightoverhead = FAHeightIncrPer_first_r_port +
                             (ERP - 1) * FAHeightIncrPer_later_r_port;
            ;
          } else {
            if ((RWP == 0) && (EWP == 1)) {
  ...
  faramcell.height = ceil((double)(noof_rowsdata) / 16.0) * stitch_ramv +
                     (CAM2x2Height_1p + 2 * heightoverhead) *
  ...
  */
  faramcell.width =
      noof_colnsdata *
          (BitWidth1x1 + (Widthtrack * 2 * (RWP + (ERP - NSER) + EWP - 1) +
                          Widthtrack * NSER)) +
  ...
  FAarea.height = faramcell.height + precharge * (RWP + EWP) +
                  SenseampHeight * (RWP + ERP) + DatainvHeight * (RWP + EWP) +
                  FAOutdriveHeight * (RWP + ERP);
  ...
  // area_all_dataramcells =
  // Ndbl * calculate_area (faramcell, techscaling_factor) *
  // CONVERT_TO_MMSQUARE;
  // faarea_all_subarrays =
  // Ndbl * calculate_area (FAarea, techscaling_factor) * CONVERT_TO_MMSQUARE;
  area_all_dataramcells =
      Ndbl * faramcell.height * faramcell.width * CONVERT_TO_MMSQUARE;
  faarea_all_subarrays =
      Ndbl * FAarea.height * FAarea.width * CONVERT_TO_MMSQUARE;
  return (FAarea);
}
...
area_type faarea(int baddr, int b0, int C, int B, int Ndbl, int RWP, int ERP,
  ...
  // v4.1: Fixing double->int type conversion problems. EPSILON is added below
  // to make sure the final int value is the correct one blocksel = MAX
  // (logtwo_area ((double) (B)), (8 * B) / b0);
  blocksel = MAX((int)(logtwo_area((double)(B)) + EPSILON), (8 * B) / b0);
  blocksel = (blocksel > tracks_outdrvfanand_p)
                 ? (blocksel - tracks_outdrvfanand_p)
                 : 0;
  ...
  temp_aspect = ((temp.height / temp.width) > 1.0)
                    ? (temp.height / temp.width)
                    : 1.0 / (temp.height / temp.width);
  aspect_ratio_data = ((fa_area.height / fa_area.width) > 1.0)
                          ? (fa_area.height / fa_area.width)
                          : 1.0 / (fa_area.height / fa_area.width);
  if (aspect_ratio_data > temp_aspect) {
...
void fatotalarea(int baddr, int b0, int Ndbl, parameter_type *parameters,
  ...
  faresult->tagoutdrvdecode_area = muxdriverdecode(
      parameters->block_size, b0, parameters->num_readwrite_ports,
      parameters->num_read_ports, parameters->num_write_ports);
  ...
  faresult->totalarea =
      faresult->dataarray_area.height * faresult->dataarray_area.width +
      faresult->datapredecode_area.height * faresult->datapredecode_area.width +
      faresult->datacolmuxpredecode_area.height *
          faresult->datacolmuxpredecode_area.width +
      faresult->datacolmuxpostdecode_area.height *
          faresult->datacolmuxpostdecode_area.width +
      ...
      faresult->tagcolmuxpredecode_area.height *
          faresult->tagcolmuxpredecode_area.width +
      faresult->tagcolmuxpostdecode_area.height *
          faresult->tagcolmuxpostdecode_area.width +
      faresult->tagoutdrvdecode_area.height *
          faresult->tagoutdrvdecode_area.width +
...
void area_subbanked(int baddr, int b0, int RWP, int ERP, int EWP, int Ndbl,
  ...
  if (parameters->fully_assoc == 0) {
    area(baddr, b0, Ndbl, Ndwl, Nspd, Ntbl, Ntwl, Ntspd, NSubbanks, parameters,
         &result_area);
  } else {
  ...
  if (NSubbanks == 2) {
    ...
    total_width =
        (result_area.dataarray_area.width + result_area.tagarray_area.width) *
            2 +
        (ADDRESS_BITS + BITOUT) * NSubbanks * (RWP + ERP + EWP);
  }
  if (NSubbanks == 4) {
    total_height = 2 * result_area.dataarray_area.height +
                   2 * (RWP + ERP + EWP) * ADDRESS_BITS;
    total_width =
        (result_area.dataarray_area.width + result_area.tagarray_area.width) *
            2 +
        (ADDRESS_BITS + BITOUT) * NSubbanks * (RWP + ERP + EWP);
  ...
  if (NSubbanks == 8) {
    total_height =
        (result_area.dataarray_area.width + result_area.tagarray_area.width) *
            2 +
        (ADDRESS_BITS + BITOUT) * NSubbanks * (RWP + ERP + EWP) * 0.5;
    total_width = 2 * (2 * result_area.dataarray_area.height +
                       2 * (RWP + ERP + EWP) * ADDRESS_BITS) +
                  (ADDRESS_BITS + BITOUT) * NSubbanks * (RWP + ERP + EWP);
  }
  ...
  if (NSubbanks > 8) {
    ...
    htree = (int)(logtwo_area((double)(blocks)) + EPSILON);
    base_height =
        2 * ((result_area.dataarray_area.width +
              result_area.tagarray_area.width) *
                 2 +
             (ADDRESS_BITS + BITOUT) * 16 * (RWP + ERP + EWP) * 0.25) +
        (ADDRESS_BITS + BITOUT) * 16 * (RWP + ERP + EWP) * 0.5;
    base_width = 2 * (2 * result_area.dataarray_area.height +
                      2 * (RWP + ERP + EWP) * ADDRESS_BITS) +
                 (ADDRESS_BITS + BITOUT) * 16 * (RWP + ERP + EWP) * 0.25;
    base_subbanks = 16;
    if (htree % 2 == 0) {
    ...
    if (iter_height == 0) {
      ...
      for (i = 1; i <= iter_height; i++) {
        total_height = 2 * (inter_height) + (ADDRESS_BITS + BITOUT) * 4 *
                                                inter_subbanks *
                                                (RWP + ERP + EWP) * 0.5;
        inter_height = total_height;
    ...
    if (iter_width == 0) {
      ...
      if (htree % 2 == 0) {
        for (i = 1; i <= iter_width; i++) {
          total_width = 2 * (inter_width) + (ADDRESS_BITS + BITOUT) *
                                                inter_subbanks *
                                                (RWP + ERP + EWP);
  ...
  aspect_ratio_total = (aspect_ratio_total > 1.0) ? (aspect_ratio_total)
                                                  : 1.0 / (aspect_ratio_total);
  ...
  aspect_ratio_subbank =
      (result_area.dataarray_area.height /
       (result_area.dataarray_area.width + result_area.tagarray_area.width));
  aspect_ratio_subbank = (aspect_ratio_subbank > 1.0)
                             ? (aspect_ratio_subbank)
                             : 1.0 / (aspect_ratio_subbank);
  aspect_ratio_total = (result_subbanked->height / result_subbanked->width);
  aspect_ratio_total = (aspect_ratio_total > 1.0) ? (aspect_ratio_total)
                                                  : 1.0 / (aspect_ratio_total);
}
...
int data_organizational_parameters_valid(int B, int A, int C, int Ndwl,
  ...
  if (assoc == 0) {
    ...
    if (C / (B * A * Ndbl * Nspd) <= 0) {
      return (FALSE);
    }
    if ((8 * B * A * Nspd / Ndwl) <= 0) {
      return (FALSE);
    }
    ...
    // v4.1: Fixing double->int type conversion problems. EPSILON is added below
    // to make sure the final int value is the correct one data_rows =
    // C/(B*A*Ndbl*Nspd) ;
    ...
    // Added by Shyam to specify a minimum 8KB mat (subarray) size in the data
    // array if((C <= 8192)&&((Ndwl > 1)||(Ndbl > 1))) return (FALSE); End of
    // Shyam's minimum mat size change
...
int tag_organizational_parameters_valid(int B, int A, int C, int Ntwl, int Ntbl,
  ...
  if (assoc == 0) {
    ...
    if (C / (B * A * Ntbl * Ntspd) <= 0) {
      return (FALSE);
    }
    if ((8 * B * A * Ntspd / Ntwl) <= 0) {
      return (FALSE);
    }
    /*dt: Don't want ridicolously small arrays*/
    ...
    // Shyam for vX.X: tag_rows = number of sets is max when Ntbl=Ntspd=1.
    // Checking that tagrows is at least 1 instead of 8. FIX THIS LATER
    if (8 > tag_rows) {
      return (FALSE);
    }
